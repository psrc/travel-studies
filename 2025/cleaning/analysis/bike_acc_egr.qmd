...
source("functions.R")
# before 10/20/2025 when mode_acc and mode_egr code is wrong and new delivered fixes are incorporated
# fixes: (fix table in codebook rsg_value_labels_initial_fix.csv)
# 1. mode_acc_fix
# 2. mode_egr_fix
# 3. school_freq_fix
tables <- readRDS("analysis_table_before_mode_fix.RDS")
...
person <- tables[["person"]] %>%
  select(all_of(c("person_id","relationship","age_detailed","bike_freq","school_freq_fix"))) %>%
...
## Solution from codebook correction [database not changed yet]
...
```{r}
CreateTableOne(data = transit_trips,
                         vars = c("mode_acc_fix","mode_egr_fix"), 
                         factorVars = c("mode_acc_fix","mode_egr_fix"), 
                         includeNA = TRUE)
```
...
df <- transit_trips %>%
  mutate(across(all_of(c("mode_acc_fix","mode_egr_fix")), ~get_labels(., varname = cur_column(), "fix", order = TRUE)))
