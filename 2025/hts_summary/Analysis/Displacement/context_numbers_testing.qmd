...
# additional testing -- new code (2026/05)
...
# additional testing -- new code (2026/05)
data_2023_test <- hts_data$hh %>% 
  filter(survey_year==2023)
table(data_2023_test$displaced, data_2023_test$res_dur)

data_2023_test_1 <- data_2023_test %>% 
  filter(movers == "Did move")
table(data_2023_test_1$displaced, data_2023_test_1$res_dur)
table(data_2023_test_1$displaced, data_2023_test_1$prev_home_wa) #2 were displaced and prev home in another state
table(hts_data$hh$displaced, hts_data$hh$prev_home_wa) #2 were displaced and prev home in another state
# checking out the 2 households that were displaced but their prev home was in another state
data_2023_test_2 <- data_2023_test_1 %>% 
  filter(prev_home_wa == "No, previous home was in another state",
         displaced == "Displaced")

# checking the other years
data_2019_test <- hts_data$hh %>% 
  filter(survey_year==2019)
data_2021_test <- hts_data$hh %>% 
  filter(survey_year==2021)
data_2025_test <- hts_data$hh %>% 
  filter(survey_year==2025)

table(data_2019_test$displaced, data_2019_test$prev_home_wa) #displaced only from WA
table(data_2021_test$displaced, data_2021_test$prev_home_wa) #displaced only from WA
table(data_2025_test$displaced, data_2025_test$prev_home_wa) #displaced only from WA
...
# 14,880 total households before filter
hts_data$hh <- hts_data$hh %>%
  filter(movers == "Did move")
# 7,548 total MOVING households after filter
```
...
hts_data$hh <- hts_data$hh %>% 
  filter(survey_year!=2017)
# 5,574 total non-2017, MOVING households after filter
...
basics <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced"~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year!='2017')
# 11,605 total households

# new code (2026/05) -----
table(basics$displaced, basics$res_dur)

basics_2019 <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced"~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year=='2019')
table(basics_2019$displaced, basics_2019$res_dur)
basics_2021 <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced"~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year=='2021')
table(basics_2021$displaced, basics_2021$res_dur)
basics_2023 <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced"~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year=='2023')
table(basics_2023$displaced, basics_2023$res_dur)
basics_2025 <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced"~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year=='2025')
table(basics_2025$displaced, basics_2025$res_dur)

table(basics$displaced, basics$prev_home_wa) 

# new code (2026/05)
# potential other issue in 2023
basics <- hts_data$hh %>% 
  mutate(sample= case_when(displaced=="Displaced" & grepl("Yes", prev_home_wa) ~"Displaced",
                           TRUE~"No")) %>% 
  filter(survey_year!='2017')
# 11,605 total households

table(basics$displaced, basics$res_dur)
table(basics$sample, basics$prev_home_wa) 
...
```
### using new method to test proportions (2026/05)
New method is using psrc_hts_stat() instead of the summarize_weighted(), which is included in the stats_context() and the stats_context_geo() - after re-running/generating these values, the proportions weren't lining up even though the count and estimate numbers were. This could be aresult of the way that the proportions are calculated
```{r}
# this function generates numbers for the region - combined survey years and separate
stats_context_new <- function(variable_name=NULL){
  ...
  hhx[, survey_year:=if_else(survey_year %in% c(2019,2021,2023,2025), "combined", as.character(survey_year))]
  hhx[survey_year=="combined", hh_weight:=hh_weight/4]
...
```{r}
displaced_newmethod <- stats_context_new('Displaced') 

# manually trying new method
hts_data <- get_psrc_hts(survey_vars = vars)  # default includes all survey_years

# trial 2 (original code: line 658)
hts_data$hh <- hts_data$hh %>% 
  mutate(movers=case_when(res_dur=="Less than a year" & grepl("Yes", prev_home_wa)|
                            res_dur=="Between 1 and 2 years" & grepl("Yes", prev_home_wa)| 
                            res_dur=="Between 2 and 3 years" & grepl("Yes", prev_home_wa)|
                            res_dur=="Between 3 and 5 years" & grepl("Yes", prev_home_wa) ~ 'Did move',
                          # res_dur=="Missing Response" ~ 'No response',
                          TRUE ~ 'Did not move'),
         displaced=case_when(prev_res_factors_forced=='Selected'|
                               prev_res_factors_income_change=='Selected'|
...
# testing basic survey logic
hh_2023 <- hts_data$hh %>% 
  filter(survey_year=='2023')
table(hh_2023$res_dur,hh_2023$prev_home_wa) #checking if all hh asked of prev_home_wa, regardless of res_dur?
table(hh_2023$prev_home_wa,hh_2023$prev_res_factors_forced)
table(hh_2023$res_dur,hh_2023$prev_res_factors_forced) #checking if all hh asked of prev_res_factors, regardless of res_dur

filter_23 <- hh_2023 %>%
  filter(res_dur=="Between 5 and 10 years" |
           res_dur=="Between 10 and 20 years"|
           res_dur=="More than 20 years" &
           prev_home_wa=="Yes, previous home was in Washington") %>% 
  select(hh_id, res_dur, prev_home_wa, prev_res_factors_forced)
filter_23_hhid <- filter_23 %>% 
  filter(!is.na(prev_home_wa)) %>% 
  select(hh_id)
print(filter_23_hhid$hh_id)

filter_23_2 <- hh_2023 %>%
  filter(prev_home_wa=="No, previous home was in another state"|
           prev_home_wa=="No, previous home was in another country") %>% 
  select(hh_id, res_dur, prev_home_wa, prev_res_factors_forced)
filter_23_2_hhid <- filter_23_2 %>% 
  filter(!is.na(prev_res_factors_forced)) %>% 
  select(hh_id)
print(filter_23_2_hhid$hh_id)

# testing how many hh moved from outside of wa
filter_move <- hts_data$hh %>% 
  filter(res_dur=="Less than a year" |
           res_dur=="Between 1 and 2 years" |
           res_dur=="Between 2 and 3 years" |
           res_dur=="Between 3 and 5 years")
table(filter_move$survey_year)
filter_move_notwa <- filter_move %>% 
  filter(prev_home_wa!="Yes, previous home was in Washington")
table(filter_move_notwa$survey_year)
filter_move_notwa_nores <- filter_move %>% 
  filter(prev_home_wa!="Yes, previous home was in Washington" & 
           is.na(prev_res_factors_forced))
table(filter_move_notwa_nores$survey_year)


hh_2025 <- hts_data$hh %>% 
  filter(survey_year=='2025')
table(hh_2025$res_dur,hh_2025$prev_home_wa) #checking if all hh asked of prev_home_wa, regardless of res_dur?
table(hh_2025$prev_home_wa,hh_2025$prev_res_factors_forced)
table(hh_2025$res_dur,hh_2025$prev_res_factors_forced) #checking if all hh asked of prev_res_factors, regardless of res_dur
...
filter_25 <- hh_2025 %>%
  filter(res_dur=="Between 5 and 10 years" |
           ...
           prev_home_wa=="Yes, previous home was in Washington") %>% 
  select(hh_id, res_dur, prev_home_wa, prev_res_factors_forced)
filter_25_hhid <- filter_25 %>% 
  filter(!is.na(prev_home_wa)) %>% 
  select(hh_id)
print(filter_25_hhid$hh_id)

filter_25_2 <- hh_2025 %>%
  filter(prev_home_wa=="No, previous home was in another state"|
           prev_home_wa=="No, previous home was in another country") %>% 
  select(hh_id, res_dur, prev_home_wa, prev_res_factors_forced)
filter_25_2_hhid <- filter_25_2 %>% 
  filter(!is.na(prev_res_factors_forced)) %>% 
  select(hh_id)
print(filter_25_2_hhid$hh_id)

hh_2019 <- hts_data$hh %>% 
  filter(survey_year=='2019')
table(hh_2019$res_dur,hh_2019$prev_home_wa) #checking if all hh asked of prev_home_wa, regardless of res_dur?
table(hh_2019$res_dur,hh_2019$prev_res_factors_forced) #checking if all hh asked of prev_res_factors, regardless of res_dur

hh_2021 <- hts_data$hh %>% 
  filter(survey_year=='2021')
table(hh_2021$res_dur,hh_2021$prev_home_wa) #checking if all hh asked of prev_home_wa, regardless of res_dur?
table(hh_2021$res_dur,hh_2021$prev_res_factors_forced) #checking if all hh asked of prev_res_factors, regardless of res_dur


# trial 3 (original code: line 658)
hts_data$hh <- hts_data$hh %>% 
  mutate(movers=case_when(res_dur=="Less than a year" & grepl("Yes", prev_home_wa)|
                            res_dur=="Between 1 and 2 years" & grepl("Yes", prev_home_wa)| 
                            res_dur=="Between 2 and 3 years" & grepl("Yes", prev_home_wa)|
                          ...
                          # res_dur=="Missing Response" ~ 'No response',
                          TRUE ~ 'Did not move'),
         displaced=case_when(prev_res_factors_forced=='Selected' & movers=="Did move"|
                               prev_res_factors_income_change=='Selected' & movers=="Did move"|
                               prev_res_factors_housing_cost =='Selected' & movers=="Did move"|
                               prev_res_factors_community_change=='Selected' & movers=="Did move"~ 'Displaced',
                             TRUE~'Not displaced')) %>% 
  filter(survey_year=='2025')

new_method_test <- psrc_hts_stat(hts_data, "hh", c("movers","displaced"), incl_na = TRUE)

# who are the 8 who 'did not move' but were 'displaced' in 2023?
new_method_test_filter <- hts_data$hh %>% 
  filter(movers=="Did not move" & displaced=="Displaced")

wb <- write.xlsx(new_method_test_filter, file = 'T:/60day-TEMP/Mary/HHTS/logic_hh_2325.xlsx',
                 colNames = TRUE,
                 overwrite = TRUE)
# who is the 1 who 'did not move' but was 'displaced' in 2025?
