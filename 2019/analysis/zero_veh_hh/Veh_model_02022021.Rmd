...
library(stargazer)
library(MASS)
library(caret) #requires lattice and ggplot2, for re-sampling models
library(lattice)
...
\
\
**Vehicle count by housing tenure - survey sample**
```{r}
table1(~rent_own_simp|vehicle_group, data=person_and_household)
```
\
\
**Vehicle count by housing tenure - region (weighted)**
...
\
**Region (weighted)**  
```{r tenure vehicle ownership plot - weighted}
hh_vehcount_rentown <- person_and_household %>%
  filter(!is.na(rent_own_simp)) %>%
  group_by(rent_own_simp,vehicle_group) %>%
  summarise(n=n(), HouseholdWeight = sum(hh_wt_combined))
...
ggplot(data = hh_vehcount_rentown, aes(x=rent_own_simp, y=HouseholdWeight, fill= vehicle_group)) +
  geom_bar(stat="identity") +
  theme(axis.text.x=element_text(angle=90, hjust=1, vjust=0.5)) +
  labs(x = "Household Tenure", 
       y = "Region (weighted)", 
...
cor_matrix_pvalues <-round(cor_matrix_pvalues$P,3)

# Hide upper triangle
upper<-cor_matrix_pvalues
upper[upper.tri(cor_matrix_pvalues)]<-""
upper<-as.data.frame(upper, nrow=15, ncol=15)
print.data.frame(upper)
```
\
...
PseudoR2 <- PseudoR2(ordered_logit_all, "all")
PseudoR2
```

```{r McFadden adj R2 model, include=FALSE, eval=FALSE}
PseudoR2(ordered_logit_all, which="McFaddenAdj")
# returns NA
```
\
...
Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated R2 full model}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
# the number of parameters includes interaction terms, random effects, dummy variables standing for non-reference categories of a nominal predictor, polynomials, etc.
n_parameters <- 39 # or 8
```
*McFadden's* $R^{2}$
```{r}
Mc_R2 <- 1-(loglikelihood_fitted/loglikelihood_restricted)
Mc_R2
```

*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_full <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_full
```
\
\
...
Of the SES-related variables, the number of household licenses (`driver_simp`) is most highly correlated with vehicle count (*0.57 = large correlation*). As a result, **number of household licenses** has the best explanatory power and will be included in the model as the SES-variable.
\
\
When compared to the other SES-related variables, the number of household licenses (`driver_simp`) is most correlated with household size (`hhsize_simp_num`; *0.81 = large correlation*) and the number of household workers (`numworkers_simp`; *0.48 = moderate correlation*), so these will not be included in the same model but can be used as alternatives. 
\
\
Jobs per worker (`jobs_numworkers`) and number of workers (`numworkers_simp`) are also highly correlated (*0.74 = large correlation*) because of the way that the former was calculated. Out of these two variables, vehicle count is more highly correlated with the number of workers *(0.29 = small correlation)* than jobs per worker *(0.14 = small correlation)*. Both number of workers and jobs per worker are correlated with number of household licenses (*0.48 = moderate correlation* and *0.26 = small correlation*, respectively). Both are correlated with number of household size (*0.48 = moderate correlation* and *0.25 = small correlation*, respectively). Because of these correlations, neither will be included in the model. 
\
\
...
Because these variables are not included in the correlation matrix, finding categorical variables with explanatory power requires more interpretation of the values that reflect significance (t- and p-values) from the full model (with all variables). 
\
\
**Explanatory power**
...
PseudoR2 <- PseudoR2(ordered_logit_1, "all")
PseudoR2
```

```{r McFadden adj R2 model1, include=FALSE, eval=FALSE}
PseudoR2(ordered_logit_1, "McFaddenAdj")
# returns NA
```

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated R2 model1}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]

# the number of parameters includes interaction terms, random effects, dummy variables standing for non-reference categories of a nominal predictor, polynomials, etc.
n_parameters <- 16 # or 8
```
*McFadden's* $R^{2}$
```{r}
Mc_R2 <- 1-(loglikelihood_fitted/loglikelihood_restricted)
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_1 <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_1
```
\
\
...
PseudoR2 <- PseudoR2(ordered_logit_2, "all")
PseudoR2
```

```{r McFadden adj R2 model2, include=FALSE, eval=FALSE}
PseudoR2(ordered_logit_2, "McFaddenAdj")
# returns NA
```
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated R2 model2}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]

# the number of parameters includes interaction terms, random effects, dummy variables standing for non-reference categories of a nominal predictor, polynomials, etc.
n_parameters <- 21 # or 10
```
*McFadden's* $R^{2}$
```{r}
Mc_R2 <- 1-(loglikelihood_fitted/loglikelihood_restricted)
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_2 <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_2
```
\
\
...
**Explanatory factor - residential type**  
...
#### <span style="color:#009D6A">*Model 1a (with race)*</span>
...
PseudoR2 <- PseudoR2(ordered_logit_1_race, "all")
PseudoR2
```

```{r McFadden adj  R2 model1 with race, include=FALSE, eval=FALSE}
PseudoR2(ordered_logit_1_race, "McFaddenAdj")
# returns NA
```
<span style="color:#20b2aa">Race in this model (Model 1a) is significant. Households of color (in reference to White only households) are more likely to own fewer vehicles. The strength of this association may be impacted by the "Missing/Other" race category. 
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated R2 model1 with race}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]

# the number of parameters includes interaction terms, random effects, dummy variables standing for non-reference categories of a nominal predictor, polynomials, etc.
n_parameters <- 18 # or 9
```
*McFadden's* $R^{2}$
```{r}
Mc_R2 <- 1-(loglikelihood_fitted/loglikelihood_restricted)
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_1a <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_1a
```
<span style="color:#20b2aa">Based on the McFadden values, this model is a slightly worse fit than the full model, but a better fit than Model 1 and Model 2. 
\
\
This model is a slightly better fit than Model 1 or Model 2 according to McFadden. Based on AIC values it is slightly better than Model 1 and worse than Model 2. Based on BIC values it is slightly worse than Model 1 and worse than Model 2.</span>
\
\

#### <span style="color:#009D6A">*Model 2a (with race)*</span>
...
PseudoR2 <- PseudoR2(ordered_logit_2_race, "all")
PseudoR2
```

```{r McFadden adj R2 model2 with race, include=FALSE, eval=FALSE}
PseudoR2(ordered_logit_2_race, "McFaddenAdj")
# returns NA
```
<span style="color:#20b2aa">Race in this model (Model 2a) is significant - less significant than in Model 1a, but the odds ratio is higher. Households of color (in reference to White only households) are more likely to own fewer vehicles. The strength of this association may be impacted by the "Missing/Other" race category.
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model2 with race}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]

# the number of parameters includes interaction terms, random effects, dummy variables standing for non-reference categories of a nominal predictor, polynomials, etc.
n_parameters <- 23 # or 11
```
*McFadden's* $R^{2}$
```{r}
Mc_R2 <- 1-(loglikelihood_fitted/loglikelihood_restricted)
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_2a <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_2a
```
<span style="color:#20b2aa">Based on the McFadden values, this model is a slightly worse fit than the full model and Model 1a, but a better fit than Model 1 and Model 2. 
\
\
This model is a slightly worse fit than Model 1 and slightly better than Model 2 according to McFadden. Based on AIC values it is slightly better than Models 1, 1a, and 2. Based on BIC values it is slightly worse than Model 2, and worse than Models 1 and 1a.</span>
\
\
...
* Household size and income (see below)
\
\

#### <span style="color:#009D6A">Household Income and Size</span> 
##### <span style="color:#486CAB">*Model 3a*</span>
This model is an iteration of <a href="#model-1a-with-race">Model 1a</a> (most restrictive set of variables, with race), but includes an interaction between household income and household size.
```{r hh income and size 3a: analysis}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit_3a <-polr(as.factor(vehicle_group) ~
                          hhincomeb_reordered*hhsize_simp_num +
                          hh_lifecycle +
                          driver_simp +
                          numworkers_simp +
                          hh_race_2 +
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_logit_3a)),2)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit_3a)),2)
# combined table
ctable <-cbind(ctable, "p value"=p, "odds ratio"=oddsratio)
ctable
```

```{r McFadden R2 model3a}
PseudoR2 <- PseudoR2(ordered_logit_3a, "all")
PseudoR2
```
<span style="color:#20b2aa">In Model 3a, there is no significant interaction between household income and household size.
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model3a}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_3a <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_3a
```
\
\

##### <span style="color:#486CAB">*Model 3b*</span>
This model is an iteration of <a href="#model-2a-with-race">Model 2a</a> (expanded set of variables, with race), but includes an interaction between household income and household size.
```{r hh income and size 3b: analysis}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit_3b <-polr(as.factor(vehicle_group) ~
                          hhincomeb_reordered*hhsize_simp_num +
                          hh_lifecycle +
                          driver_simp +
                          numworkers_simp +
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_logit_3b)),2)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit_3b)),2)
# combined table
ctable <-cbind(ctable, "p value"=p, "odds ratio"=oddsratio)
ctable
```

```{r McFadden R2 model3b}
PseudoR2 <- PseudoR2(ordered_logit_3b, "all")
PseudoR2
```
<span style="color:#20b2aa">In Model 3b, there is no significant interaction between household income and household size.
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model3b}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_3b <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_3b
```
\
\

##### <span style="color:#486CAB">*Model 3c*</span>
Although there was no interaction found between household income and household size when tested within Models 3a and 3b, this may be due to the other variables, such as `driver_simp` (number of household licenses) or `numworkers_simp` (number of household workers) being too highly correlated with the household size variable. 

As described in <a href="#numerical-variables">Section 5: Numerical variables</a>, household size is most correlated with the number of household licenses (0.81 = large correlation). Household size is also correlated with the number of household workers (0.48 = moderate correlation), as shown through <a href="#correlation-coefficients">Section 4: Correlation coefficients</a>.

Because Model 3a is a better fit than Model 3b, based on McFadden's adjusted value, Model 3a will be used as the base of Model 3c to further test this interaction. The main difference between Models 3a and 3c is that Model 3c will exclude `driver_simp` and `numworkers_simp`.
```{r hh income and size 3c: analysis}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit_3c <-polr(as.factor(vehicle_group) ~
                          hhincomeb_reordered*hhsize_simp_num +
                          hh_lifecycle +
                          hh_race_2 +
                          rent_own_simp +
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_logit_3c)),2)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit_3c)),2)
# combined table
ctable <-cbind(ctable, "p value"=p, "odds ratio"=oddsratio)
ctable
```

```{r McFadden R2 model3c}
PseudoR2 <- PseudoR2(ordered_logit_3c, "all")
PseudoR2
```
<span style="color:#20b2aa">In Model 3c, there is still no significant interaction between household income and household size, however it is closer to a significant relationship than in Models 3a and 3b. 
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model3c}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_3c <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_3c
```
<span style="color:#20b2aa">Based on the McFadden and McFadden adjusted values, Model 3c is a worse fit than Models 3a and 3b. It has the lowest values of any of the models tested so far. 
\
\

##### <span style="color:#486CAB">*Model 3d*</span>
This is an iteration of Model 3a, but includes household education minimum (`hh_edu_min_simp`) as a variable. Education minimum is included because it had more explanatory power than education maximum in <a href="#model-with-all-variables">Section 5: Model with all variables</a>.
\
\
Like Model 3a and 3b, and unlike Model 3c, it includes number of household licenses (`driver_simp`) and number of household workers (`numworkers_simp`). Although these latter variables are highly correlated with household size, removing them in Model 3c resulted in the lowest McFadden adjusted values of all previous models. 
```{r hh income and size 3d}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit <-polr(as.factor(vehicle_group) ~
                       hhincomeb_reordered*hhsize_simp_num +
                       hh_lifecycle +
...
```{r McFadden R2 model3d}
PseudoR2 <- PseudoR2(ordered_logit, "all")
PseudoR2
```
<span style="color:#20b2aa">In Model 3d, there is no significant interaction between household income and household size. 
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model3d}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_3d <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_3d
```
<span style="color:#20b2aa">Based on the McFadden and McFadden adjusted values, Model 3d is a better fit than all of the previous models, except for the model full model (with all variables). 
\
\


##### <span style="color:#486CAB">*Model 3e*</span>
This is an iteration of Model 3b, but includes household education minimum (`hh_edu_min_simp`) as a variable. 
\
\
Like Model 3a and 3b, and unlike Model 3c, it includes number of household licenses (`driver_simp`) and number of household workers (`numworkers_simp`). Although these latter variables are highly correlated with household size, removing them in Model 3c resulted in the lowest McFadden adjusted values of all previous models. 
```{r hh income and size 3e}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit <-polr(as.factor(vehicle_group) ~
                       hhincomeb_reordered*hhsize_simp_num +
                       hh_lifecycle +
...
```{r McFadden R2 model3e}
PseudoR2 <- PseudoR2(ordered_logit, "all")
PseudoR2
```
<span style="color:#20b2aa">In Model 3e, there is still no significant interaction between household income and household size. 
\
\

Manually calculated pseudo $R^{2}$ values
```{r McFadden calculated model3e}
loglikelihood_fitted <- PseudoR2[11]
loglikelihood_restricted <- PseudoR2[12]
...
*McFadden's Adjusted* $R^{2}$
```{r}
McAdj_R2_3e <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2_3e
```
\
<span style="color:#20b2aa">Based on models 3a-3e, there is no interaction between household income and household size. 
\
\
...
*Ranked Models based on McFadden adjusted values:*

* Full model (all variables) = `r McAdj_R2_full`
* Model 3d (1a, with education) = `r McAdj_R2_3d`
* Model 1a (most restricted, with race) = `r McAdj_R2_1a`
* Model 3a (1a, hh income*hh size) = `r McAdj_R2_3a`
* Model 1 (most restricted) = `r McAdj_R2_1`
* Model 3e (1b, with education) = `r McAdj_R2_3e`
* Model 2a (slightly restricted, with race) = `r McAdj_R2_2a`
* Model 2 (slightly expanded) = `r McAdj_R2_2`
* Model 3b (2a, hh income*hh size) = `r McAdj_R2_3b`
* Model 3c (3a, further restricted) = `r McAdj_R2_3c`

\
\

### Using test and training datasets
Based on findings from the previous section, Models 1a and 2a are a better fit than Models 1 and 2. When considering McFadden and McFadden's adjusted, Model 1a is better, but when considering AIC and BIC, Model 2a is better. 


```{r eval=FALSE, include=FALSE}
# based on Section 8.6 and 9.2 https://www.tmwr.org/workflows.html#future-plans
