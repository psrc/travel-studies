...
```{r}
McAdj_R2 <- 1-((loglikelihood_fitted-n_parameters)/loglikelihood_restricted)
McAdj_R2
```
\
\
...
\
\
Based on the McFadden values, this model is a slightly worse fit than the full model, but a better fit than Model 1 and Model 2. 
\
\
...
\
\
Based on the McFadden values, this model is a slightly worse fit than the full model and Model 1a, but a better fit than Model 1 and Model 2. 
\
\
...
*Household size and income*
```{r test important interactions model 8: analysis}
# documentation about function: https://www.rdocumentation.org/packages/MASS/versions/7.3-52/topics/polr
ordered_logit <-polr(as.factor(vehicle_group) ~
