...
```
\
Because the outcome variable is household vehicle ownership, the person-level variables need to be aggregated to the household level. With this in mind, gender and employment type were removed from this analysis.  
\
...
The ordered logit model reports the effects of X on Y when “controlling” for the other independent variables. This model tests main effects - assessing the relationship between each independent variable and the dependent variable (household vehicle ownership).
...
\
\
`Jobs per worker` and `number of workers` are also highly correlated (*0.74 = large correlation*) because of the way that the former was calculated. Out of these two variables, `vehicle ownership` is more highly correlated with the `number of workers` *(0.29 = small correlation)* than `jobs per worker` *(0.14 = small correlation)*, but because `number of workers` is more correlated with `number of household licenses` (*0.48 = moderate correlation*) than `jobs per worker` (*0.26 = small correlation*), **`jobs per worker`** will be included in the model.
\
\

##### <span style="color:#486CAB">*Home/work characteristics*</span>
Of the home and work-related variables, `total auto commuting time` is most highly correlated with vehicle ownership (*0.147 = small correlation*), closely followed by `average auto commuting time` (*0.145 = small correlation*).
\
\
...
\
\
Alternatives could be `jobs accessible by auto within 30 minutes` (*-0.39 = moderate correlation*), or  `jobs accessible by transit within 45 minutes` (*-0.38 = moderate correlation*), which are the second and third most correlated accessibility variables when considering vehicle ownership, respectively.
\
\
...
* Race: Although the univariate analysis between vehicle ownership and household race (`hh_race_2`: comparing white only and POC households) does indicate a significant relationship (p<0.01), it is not significant when included in the ordered logit model.   
\
\
...
*Reference (full model with all variables):*
```{r McFadden R2 model}
...
<span style="color:#20b2aa"> Based on the McFadden and AIC/BIC values, the full model seems to have the best fit. 
\
Model 1 is a slightly better fit than Model 2 according to McFadden, but is a worse fit according to the AIC/BIC values.</span>
\
\
\

#### <span style="color:#009D6A">Example</span> 
<span style="color:#20b2aa"> The influence of residential type changes in model 2, when compared to the full model with all variables. The trend remains similar for households living in **single-family housing** - remaining significant, although becoming slightly more explanatory. </span>
...
#### <span style="color:#009D6A">*Most significant factors*</span>
...
* **Household lifecycle** 
* **Housing tenure**
* **Accessibility/proximity to transit**
* **Total commuting time**
* **Number of jobs accessible by auto (30m) or transit (45m)**
\

#### <span style="color:#009D6A">*For consideration*</span>
Based on significance or strength of impact
...
* **Housing in/out of RGC**: significant, impact - but distant measurement of accessibility
\

#### <span style="color:#009D6A">*Of interest*</span>

* **Race**: Although the univariate analysis between vehicle ownership and household race (hh_race_2: comparing white only and POC households) does indicate a significant relationship (p<0.01), it is not significant when included in the full ordered logit model. 
