...
Based on the correlation matrix, household vehicle ownership is most correlated with the number of licensed household members (0.569), household size (0.491), and transit accessibility (-0.477).
\
\
Overall, the explanatory variables can be organized into 3 categories: 

* socioeconomic characteristics  
* home and work related characteristics  
* accessibility (based on home location)
\

Although the majority of the individual interactions were significant, considering interaction effects and adding them into the model will contribute to understanding the relationship between the independent variables and household vehicle ownership. Therefore, the next step requires analysis to determine the different interactions between the variables within these categories.
...
The coefficient value represents the mean change of the dependent variable given a one-unit shift in an independent variable. It can be difficult to compare the **regular regression coefficients** because they use different scales.
...
## 6. Models
### All variables
The ordered logit model reports the effects of X on Y when “controlling” for the other independent variables. 
...
#### <span style="color:#009D6A">*Socioeconomic characteristics*</span>
##### <span style="color:#486CAB">*Variables of interest*</span>
Only the numerical variables are included in this test. 
```{r SES model : analysis}
ordered_SES <-polr(as.factor(vehicle_group) ~
                       hhsize_simp_num +
                       driver_simp +
                       numworkers_simp +
                       jobs_numworkers,
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_SES)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r SES model: stargazer table, results='asis'}
stargazer::stargazer(ordered_SES,
                     type = "html",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
...
##### <span style="color:#486CAB">*Interactions*</span>
Of the SES-related variables, number of household licenses is most highly correlated with vehicle ownership (0.57). When compared to the other SES-related variables, household licenses is highly correlated with household size (0.86). Jobs per workers and number of workers are also highly correlated (0.74).
```{r SES interactions model : analysis}
ordered_SES_int <-polr(as.factor(vehicle_group) ~
                         driver_simp*hhsize_simp_num +
                         numworkers_simp*jobs_numworkers,
                       data=person_and_household, Hess = T)

# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_SES_int)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r SES interactions model: stargazer table, results='asis'}
stargazer::stargazer(ordered_SES_int,
                     type = "html",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
\
<span style="color:#20b2aa">The coefficient of the interactive term (`driver_simp`:`hhsize_simp_num`) is negative and statistically significant at .01 level, which means that the increase of household licenses will decrease the significance effect of household size on vehicle ownership. Because the effect of household size is positive, its effect will be less positive with increasing number of drivers.
\
\
The coefficient of the interactive term (`numworkers_simp`:`jobs_numworkers`) is positive and statistically significant at the .05 level.</span>  
\
\
Plot to understand these interactions `interplot()` is used. The default value is 95% CI.  
**Household licenses and household size**  
```{r plot SES interaction 1}
interplot(m = ordered_SES_int, var1 = "hhsize_simp_num", var2 = "driver_simp" )
```
\
The plot shows that with increasing number of household licenses (along the x axis), the magnitude of the coefficient of household size decreases (along the y axis).  
\
\
**Number of household workers and jobs per worker**  
```{r plot SES interaction 2}
interplot(m = ordered_SES_int, var1 = "jobs_numworkers", var2 = "numworkers_simp")
```
\
The plot shows that with increasing number of household workers (along the x axis), the magnitude of the coefficient of the jobs:workers ratio also increases (along the y axis).  
\
\

#### <span style="color:#009D6A">*Home/work characteristics*</span>
##### <span style="color:#486CAB">*All variables*</span>
Only the numerical variables are included in this test. 
```{r home and work model : analysis}
ordered_home_work <-polr(as.factor(vehicle_group) ~
                           hh_com_auto_time_avg + hh_com_auto_time_sum +
                           hh_commutetrips,
                         data=person_and_household, Hess = T)

# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_home_work)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r home and work model: stargazer table, results='asis'}
stargazer::stargazer(ordered_home_work,
                     type = "html",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
...
##### <span style="color:#486CAB">*Interactions*</span>
Of the home and work-related variables, total commuting time by automobile is most highly correlated with vehicle ownership (0.15). Although vehicle ownership is less correlated with household commute trips (0.128), household commute trips and the number of household workers (a variable from the SES category) are highly correlated (0.70).
```{r home and work interactions model : analysis}
ordered_home_work_int <- polr(as.factor(vehicle_group) ~
                                hh_com_auto_time_avg+hh_com_auto_time_sum +
                                hh_commutetrips*numworkers_simp,
                              data=person_and_household, Hess = T)

# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_home_work_int)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r home and work interactions model: stargazer table, results='asis'}
stargazer::stargazer(ordered_home_work_int,
                     type = "html",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
\
<span style="color:#20b2aa">The coefficient of the interactive term (`hh_commutetrips`:`numworkers_simp`) is positive and statistically significant at the .1 level, which means that the coefficient of `scaled_score` depends on the value of `ln_jobs_transit_45` and vice versa. It does not indicate anything, however, about the magnitude or statistical significance of these conditional coefficients.
\
\
Although the average and total commute times are highly correlated (0.86), trying to include the interaction (`hh_com_auto_time_avg`:`hh_com_auto_time_sum`) returns an error: *"Error in polr...attemot to find suitable starting values failed Calls."* Based on the output of the model and the correlation matrix, it appears that total commute time is slightly more correlated with vehicle ownership and can be used in future modeling.</span> 
\
\

#### <span style="color:#009D6A">*Accessibility characteristics*</span>
These variables are based on home location. 
\

##### <span style="color:#486CAB">*All variables*</span>
```{r accessibility model : analysis}
ordered_access <-polr(as.factor(vehicle_group) ~
                        scaled_score +
                        ln_jobs_auto_30 + 
                        ln_jobs_transit_45 + 
                        dist_super + 
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_access)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r accessibility model: stargazer table, results='asis'}
stargazer::stargazer(ordered_access,
                     type = "html",
                     title = "Ordered Logit for Accessibility characteristics",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
\
...
##### <span style="color:#486CAB">*Interactions*</span>
Of the accessibility-related variables, transit score is most highly correlated with vehicle ownership (-0.477). When compared to the other accessibility-related variables, transit score is highly correlated with jobs accessibly by transit within 45 minutes (0.786), jobs accessible by auto within 30 minutes (0.763), and distance to supermarkets (-0.699).
```{r accessibility interactions model : analysis}
ordered_access_int <- polr(as.factor(vehicle_group) ~
                             scaled_score*ln_jobs_transit_45 +
                             ln_jobs_auto_30^2 +
                             dist_super^2 +
                             dist_school,
                           data=person_and_household, Hess = T)
...
# adding coefficients and p-values store table
ctable <- round(coef(summary(ordered_access_int)),4)
# calculate and store p values
p <- round(pnorm(abs(ctable[,"t value"]), lower.tail = F)*2,3)
# odds ratio
oddsratio <- round(exp(coef(ordered_logit)),2)
...
```{r accessibility interactions model: stargazer table, results='asis'}
stargazer::stargazer(ordered_access_int,
                     type = "html",
                     notes.append = FALSE,
                     notes =  c("<sup>&sstarf;</sup>p<0.1;<sup>&sstarf;&sstarf;</sup>p<0.05;<sup>&sstarf;&sstarf;&sstarf;</sup>p<0.01"))
```
\
<span style="color:#20b2aa">The coefficient of the interactive term (`scaled_score`:`ln_jobs_transit_45`) is negative and statistically significant at the .01 level, , which means that the increase of transit score will decrease the significance effect of jobs accessible by transit on vehicle ownership. Because the effect of job accessibility by transit is positive, its effect will be less positive with improved transit accessibility.</span> 
\
\
...
	Test every variable – which one has explanatory power?
o	Set of variables – correlation matrix – what is related? 
	Which variable expected to explain veh access/veh count
	Which related to one another?
o	Add one at a time – some may interact with others
o	Have values of coefficients changed or new signs? 
o	Instead of 2 correlated values – which of 2 does better? Hunt for explanatory power
o	Choose vehicle ownership vs. access (more complicated, adds complexity)
	Vehicle count 3+ - distinct between 2 and 3 vehicles 
	Adjusted by variable most correlated with it 
```
