...
ggplot(avg_time_by_gender, aes(x=gender, y=avg_weighted_time)) + 
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Time by Gender",
       x ="Gender Categories", 
       y = "Average Weighted Time (minutes)") +
  geom_errorbar(aes(ymin=time_gender_stats$Weighted_avg-time_gender_stats$CI_95, 
                    ymax=time_gender_stats$Weighted_avg+time_gender_stats$CI_95), 
                width=.2,
                position=position_dodge(.9))
```
\
...
## Interactions
### Commute Travel: Gender and Race
```{r, include=FALSE, eval=FALSE}
# filter only female commuters
female_commuters <- not_home_workers %>%
  filter(gender=="Female")
```

```{r, include=FALSE, eval=FALSE}
# commute distance: gender and race
weighted_stats <- not_home_workers %>%
  filter(gender== "Female" | gender== "Male") %>%
  group_by(gender, race_category) %>%
  summarise(avg_weighted_dist=round(weighted.mean(commute_auto_Distance,hh_wt_2019),2),
            avg_weighted_time=round(weighted.mean(commute_auto_time,hh_wt_2019),2))

table1(~avg_weighted_dist + avg_weighted_time | gender*race_category, 
       data=weighted_stats,
       overall=F)
```

```{r}
time_gender_race<-not_home_workers %>%
  group_by(gender, race_category) %>% 
  summarize(n=n(),
            Percent=round((n/nrow(not_home_workers))*100,2),
            HouseholdWeight=sum(hh_wt_2019),
...
ggplot(time_gender_race, aes(x=gender, y=avg_weighted_time, fill=race_category)) + 
  geom_bar(position="dodge", stat="identity") +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Time by Gender",
       x ="Gender Categories",
       y = "Average Weighted Duration (minutes)",
       fill = "Race")
```
\
\
