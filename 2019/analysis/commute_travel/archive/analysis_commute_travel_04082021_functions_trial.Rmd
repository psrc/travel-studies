...
\
\
#Testing functions with income, race aggregated, commute time
```{r}
# set up data frame with independent and dependent variables of interest
race_income_test <- not_home_workers %>%
  dplyr::select(hhincome_broad, race_agg_2, commute_auto_time)
```

```{r}
# use Two-way ANOVA function with input variabless
anova.race.income <- anova.test(race_income_test,
                                "commute_auto_time", 
                                "race_agg_2", 
                                "hhincome_broad")
```
...
```{r}
# set up data frame with independent and dependent variables of interest
comp_income_white <- not_home_workers %>%
  # establish grouping variable that results in 2 levels
  dplyr::filter(hhincome_broad=="$25,000-$49,999" | hhincome_broad=="$100,000 or more") %>%
  dplyr::filter(race_agg_2=="White Only") %>% 
  dplyr::select(hhincome_broad, race_category, commute_auto_time)
```

```{r}
# use F-test function with input variables
F.test.check(comp_income_white, 
             "commute_auto_time", 
             "hhincome_broad")
```
\
...
Welch.test.check(comp_income_white,
                 "commute_auto_time",
                 "hhincome_broad")
```
\
...
```{r}
# set up data
comp_asian_income <- not_home_workers %>%
  dplyr::filter(hhincome_broad=="$25,000-$49,999" | hhincome_broad=="$100,000 or more") %>%
  dplyr::filter(race_agg_2=="Asian POC") %>% 
  dplyr::select(hhincome_broad,  commute_auto_time)
```

```{r}
# test variance with input variables
F.test.check(comp_asian_income, 
             "commute_auto_time", 
             "hhincome_broad")
```
\
...
T.test.check(comp_asian_income, 
             "commute_auto_time", 
             "hhincome_broad")
```
...
#### Using Functions to Communte Distance, Tenure, Household Size, Income

To compare commute distance, household size, and household income 
```{r}
# set up data frame with independent and dependent variables of interest
# simplify hh size variable - need to make it categorical because "Levene's test is not appropriate with quantitative explanatory variables"
not_home_workers <- not_home_workers %>%
...
#                                 "hhsize_simp", 
#                                 "hhincome_broad")  
# ANOVA.output <- aov(commute_auto_Distance~hhsize_simp+hhincome_broad, data=hhsize_income_test)
# Tukey_test <- TukeyHSD(ANOVA.output, which="hhincome_broad")
# result<-data.frame(Tukey_test$hhincome_broad)
# p_val <- result$p.adj
#     # p_val_df <- as.data.frame(p_val)
#     # nrow(p_val_df)
#       
# sig_test <- as.data.frame(tidy(Tukey_test))
# nrow(sig_test)
# print(paste0("The Tukey HSD Test results for ", sig_test[4,2]))
# print(Tukey_test)
# 
#     #how to count items in p_val
#     for(i in 1:(nrow(sig_test))){
...
To compare commute distance between housing tenure (rent vs. own) and household and household size (2+ people) 
1. F-test
```{r}
# set up data
not_home_workers <- not_home_workers %>%
