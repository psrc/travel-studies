...
avg_dist_by_tenure <- not_home_workers %>%
  select(rent_own_simp, commute_auto_Distance, hh_wt_2019) %>%
  group_by(rent_own_simp)

temp <- avg_dist_by_tenure %>%
  filter(rent_own_simp=="Own/paying mortgage" | rent_own_simp=="Rent") %>%
  summarise(n=n(),
            Percent=round((n/nrow(avg_dist_by_tenure))*100,2),
            Group_weight = round(sum(hh_wt_2019),0),
            Weighted_percent = (round(Group_weight/sum(avg_dist_by_tenure$hh_wt_2019),4)*100),
            Weighted_avg = round(weighted.mean(commute_auto_Distance, hh_wt_2019),2),
            sd = round(sd(commute_auto_Distance),3),
            se = round(sd/sqrt(n),3),
            CI_90 = round(z*(sd/sqrt(n)),3),
...
ggplot(temp, aes(x=rent_own_simp, y=Weighted_avg, fill=)) +
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Distance by Houseing Tenure",
       x ="Simplified Tenure Categories",
       y = "Average Weighted Distance (miles)") +
  geom_errorbar(aes(ymin=Weighted_avg-CI_95,
                    ymax=Weighted_avg+CI_95),
                width=.2,
...
avg_dist_by_hhsize<-not_home_workers %>%
  select(hhsize_simp, commute_auto_Distance, hh_wt_2019) %>%
  group_by(hhsize_simp)

temp <- avg_dist_by_hhsize %>%
  summarize(n=n(),
            Percent=round((n/nrow(avg_dist_by_hhsize))*100,2),
              Group_weight = round(sum(hh_wt_2019),0),
              Weighted_percent = (round(Group_weight/sum(avg_dist_by_hhsize$hh_wt_2019),4)*100),
              Weighted_avg = round(weighted.mean(commute_auto_Distance, hh_wt_2019),2),
              sd = round(sd(commute_auto_Distance),3),
              se = round(sd/sqrt(n),3),
              CI_90 = round(z*(sd/sqrt(n)),3),
...
ggplot(temp, aes(x=hhsize_simp, y=Weighted_avg)) + 
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Distance by Household Size",
       x ="Household Size", 
       y = "Average Weighted Distance (miles)") +
  geom_errorbar(aes(ymin=Weighted_avg-CI_95, 
                    ymax=Weighted_avg+CI_95), 
...
avg_dist_by_incomecapita<-not_home_workers %>%
  select(hhincome_capita, commute_auto_Distance, hh_wt_2019) %>%
  group_by(hhincome_capita)

temp <- avg_dist_by_incomecapita %>%
  summarize(n=n(),
            Percent=round((n/nrow(avg_dist_by_incomecapita))*100,2),
              Group_weight = round(sum(hh_wt_2019),0),
              Weighted_percent = (round(Group_weight/sum(avg_dist_by_incomecapita$hh_wt_2019),4)*100),
              Weighted_avg = round(weighted.mean(commute_auto_Distance, hh_wt_2019),2),
              sd = round(sd(commute_auto_Distance),3),
              se = round(sd/sqrt(n),3),
              CI_90 = round(z*(sd/sqrt(n)),3),
...
ggplot(temp, aes(x=hhincome_capita, y=Weighted_avg)) + 
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Distance by Household Income per Capita",
...
avg_dist_by_incomehhsize1<-hh_size_1 %>%
  select(hhincome_detailed, commute_auto_Distance, hh_wt_2019) %>%
  group_by(hhincome_detailed)

temp <- avg_dist_by_incomehhsize1 %>%
  summarize(n=n(),
            Percent=round((n/nrow(avg_dist_by_incomehhsize1))*100,2),
              Group_weight = round(sum(hh_wt_2019),0),
              Weighted_percent = (round(Group_weight/sum(avg_dist_by_incomehhsize1$hh_wt_2019),4)*100),
              Weighted_avg = round(weighted.mean(commute_auto_Distance, hh_wt_2019),2),
              sd = round(sd(commute_auto_Distance),3),
              se = round(sd/sqrt(n),3),
              CI_90 = round(z*(sd/sqrt(n)),3),
...
ggplot(temp, aes(x=hhincome_detailed, y=Weighted_avg)) + 
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Distance by Household Income for Households (1 person)",
       x ="Household Income", 
       y = "Average Weighted Distance (miles)") +
  geom_errorbar(aes(ymin=Weighted_avg-CI_95, 
                    ymax=Weighted_avg+CI_95), 
...
avg_dist_by_incomehhsize2<-hh_size_2more %>%
  select(hhincome_detailed, commute_auto_Distance, hh_wt_2019) %>%
  group_by(hhincome_detailed)

temp <- avg_dist_by_incomehhsize2 %>%
  summarize(n=n(),
            Percent=round((n/nrow(avg_dist_by_incomehhsize2))*100,2),
              Group_weight = round(sum(hh_wt_2019),0),
              Weighted_percent = (round(Group_weight/sum(avg_dist_by_incomehhsize2$hh_wt_2019),4)*100),
              Weighted_avg = round(weighted.mean(commute_auto_Distance, hh_wt_2019),2),
              sd = round(sd(commute_auto_Distance),3),
              se = round(sd/sqrt(n),3),
              CI_90 = round(z*(sd/sqrt(n)),3),
...
ggplot(temp, aes(x=hhincome_detailed, y=Weighted_avg)) + 
  geom_col() +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1)) +
  labs(title="Average Auto Commute Distance by Household Income for Households (2+ people)",
       x ="Household Income", 
       y = "Average Weighted Distance (miles)") +
  geom_errorbar(aes(ymin=Weighted_avg-CI_95, 
                    ymax=Weighted_avg+CI_95), 
