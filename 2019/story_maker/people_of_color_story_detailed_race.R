...
#'race_group_detail' can ONLY be selected as the first variable
race_data <-c('race_group_detail')
...
walk<-c('mode_freq_3')
walks<-values.lu[variable=='mode_freq_3'][['value_text']]
for(this_walk in walks){
  summarize_cross_tables(race_data,c('mode_simple'),walk, this_walk)
}
race<-c('race_group_detail')
races<-values.lu[variable=='race_group_detail'][['value_text']]
for(this_race in races){
  summarize_cross_tables(c('person_dim_id'),c('mode_simple'), race, this_race)
}
