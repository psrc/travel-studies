...
- **trips per acre**
  - non-work trip/acre for metro centers is **9 times** higher than that for non-center
  - non-work trip/acre for urban centers is **7 times** higher than that for non-center
...
#| tbl-cap: weekday trip destination
kbl(
  df_plot3[,list(category,trip_type,share,count,`trip/acre`,`trip/activity unit`)] %>%
    formatting_func()
  )%>%
  kable_styling(bootstrap_options = c("striped", "hover"), html_font = "Poppins") %>%
...
1. **trips per acre**
   ...
   - especially meal trips, the centers serves about 16 times more trip/acre than non-centers

```{r}
#| out-height: 300px

plot_ly(data = df_plot,
        x = ~dest_purpose_cat, y = ~`trip/acre`,type = "bar", color = ~d_rgcname, colors = pal_rgc) %>%
  layout(xaxis = list(title = 'trip purpose'), yaxis = list(title = 'trip/acre', tickformat=".2f"), 
...
```{r}
#| tbl-cap: weekday non-work trip per acre by purpose
df_tab1 <- df_plot %>%
...
2. **trips per activity unit**
   - activity units in centers didn't generate much higher escort and social/recreation trips

```{r}
#| out-height: 300px

plot_ly(data = df_plot,
        x = ~dest_purpose_cat, y = ~`trip/activity unit`,type = "bar", color = ~d_rgcname, colors = pal_rgc) %>%
...
1. **trips per acre**
   - Metro centers serve the most trips for each non-work purpose, except for shopping purpose.   
   - Urban centers serves the most shopping trips
```{r}
#| out-height: 300px

plot_ly(data = df_plot,
        x = ~dest_purpose_cat, y = ~`trip/acre`,type = "bar", color = ~category, colors = pal_rgc_mu) %>%
  layout(xaxis = list(title = 'trip purpose'), yaxis = list(title = 'trip/acre', tickformat=".2f"), 
         title = "weekday non-work trip per acre by purpose",
         font = list(family="Poppins"))
```
...
plot_ly(data = df_plot,
        x = ~dest_purpose_cat, y = ~`trip/activity unit`,type = "bar", color = ~category, colors = pal_rgc_mu) %>%
  layout(xaxis = list(title = 'trip purpose'), yaxis = list(title = 'trip/activity unit', tickformat=".2f"), 
         title = "weekday non-work trip per activity unit by purpose",
         font = list(family="Poppins"))
```
...
## hex maps: weighted number of trips
