...
# test <- variable_list %>% group_by(variable) %>% summarise(n())
# test <- variable_list %>% filter(shared_name %in% c("mode_simple","d_rgcname","category","trip_type"))
...
sf_trip <- st_as_sf(df_trip_data_23, coords = c("dest_lng","dest_lat"),crs=4326)
sf_trip_uga <- sf_trip %>%
  st_join(., uga.lyr[,c("UGA")]) %>%
  mutate(UGA = replace_na(UGA,"not UGA")) %>%
  filter(UGA=="UGA")

df_trip_data_23_uga <- df_trip_data_23 %>% filter(trip_id %in% sf_trip_uga$trip_id)
hts_data = list(# hh = hh,
                # person = person,
                # day = day,
