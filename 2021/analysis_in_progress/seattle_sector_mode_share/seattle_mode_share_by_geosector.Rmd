...
I’m on the consultant team that is supporting the City of Seattle with their comprehensive plan update EIS. We see that mode share data from the household travel survey is available at the citywide level online, but are looking to drill down to mode shares for the sectors shown below--which I realize may involve some additional work regarding weighting/sampling at those geographies. 
...
mutate(PM_Peak=ifelse(((arrival_time>=15 & arrival_time<=18)|(depart_time>=15 & depart_time<=18)), 'PM Peak', 'Not PM Peak'))

```
Overall summary of trips
```{r}
trips_by_mode_PM<-hhts_count(mode_geo_df, group_vars=c('mode_incl_hov', 'PM_Peak'), spec_wgt='trip_weight_2017_2019_v2021')
trips_by_mode_PM
```
...
```
There are 66,832 trip origins in City of Seattle. 43,757 of the trips are not in the PM Peak, 23,067 are in the PM Peak.
```{r}
...
```{r}
od_sector_sea<-merge(origin_sector_sea, dest_sector_sea, by='trip_id', suffixes=c('', '_dest'))%>% mutate(mode_nonm=ifelse((mode_incl_hov=='Walk' | mode_incl_hov=='Bike') , 'Non-Motorized', mode_incl_hov))
```
...
```{r}
trips_by_mode_PM<-hhts_count(od_sector_sea, group_vars=c('mode_nonm', 'PM_Peak', 'area', 'area_dest'), spec_wgt='trip_weight_2017_2019_v2021')
```
...
```{r}
write.csv(trips_by_mode_PM, 'trips_by_mode_all.csv')
```
...
```{r}
od_sector_sea_only<- od_sector_sea %>% drop_na(area)

all_day_trips_Seattle_origin<-hhts_count(od_sector_sea_only, group_vars=c('area', 'mode_nonm' ),spec_wgt='trip_weight_2017_2019_v2021', incl_na=FALSE)%>%filter(mode_nonm!='Total')
...
```{r}
create_facet_bar_chart(all_day_trips_Seattle_origin, w.x='mode_nonm', w.y='share', g='area', f='mode_nonm', w.scales='fixed')+theme(panel.spacing.y = unit(1, "lines"))
```
