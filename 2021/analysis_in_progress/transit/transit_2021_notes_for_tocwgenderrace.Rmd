...
detailed_trips_by_mode_17_19<-hhts_count(transit_data_17_19_only, group_vars='BusorNot', spec_wgt='trip_adult_weight_2017_2019')%>%filter(BusorNot!='Total')
detailed_trips_by_mode_21<-hhts_count(transit_data_21_only, group_vars='BusorNot', spec_wgt='trip_adult_weight_2021')%>%filter(BusorNot!='Total')
...
transit_summs_2017_2019<-hhts_count(transit_data_17_19, group_vars=c(g,'mode_simple'), spec_wgt='trip_adult_weight_2017_2019')%>%
  filter(cat_name!='Total') %>%
  filter(mode_simple=='Transit')
transit_summs_2021<-hhts_count(transit_data_21, group_vars=c(g,'mode_simple'), spec_wgt='trip_adult_weight_2021')%>%
  filter(cat_name!='Total') %>%
  filter(mode_simple=='Transit')
...
p<-static_column_chart(t=transit_summs_long, w.x=get(g), w.y='share', f=survey, w.moe='share_moe', est.type='percent', w.color = 'LtOrDkOr')+coord_flip()
...
p<-static_column_chart(t=transit_summs_long, w.x=cat_name, w.y='share', f='survey', w.moe='share_moe', est.type='percent', w.color = 'LtOrDkOr')+coord_flip()
...
p<-static_column_chart(t=transit_summs_long, w.x='race_eth_broad', w.y='share', f='survey', w.moe='share_moe', est.type='percent', w.color = 'LtGnDkGn')
...
for(i in seq(1, length(simple_groupings))){
  ...
  transit_summs_2017_2019 <- hhts_count(transit_data_17_19, 
                                        group_vars=c('gender', 'race_eth_broad','mode_simple'), 
                                        spec_wgt='trip_adult_weight_2017_2019')%>%
    filter(cat_name!='Total') 
...
for(g in groupings){
  cat_name<- eval(g)
  transit_summs_2017_2019<-hhts_count(transit_data_17_19_only, group_vars=cat_name, spec_wgt='trip_adult_weight_2017_2019')%>%
    filter(get(g)!='Total')
  
  transit_summs_2021<-hhts_count(transit_data_21_only, group_vars=cat_name, spec_wgt='trip_adult_weight_2021')%>%
    filter(get(g)!='Total')
