...
psurvey_17_simp <- psurvey_17 %>%
  ...
  mutate(transit_mode = factor(case_when(commute_mode=="Bus (public transit)" |
                                         ...
                                         is.na(commute_mode) ~ "4-No commute",
                                         TRUE ~ "3-Other modes"))) 

# psurvey_17_simp$transit_mode <- factor(psurvey_17_simp$transit_mode,
#                                        levels=c("Active/micro-transit",
#                                                 "Transit",
#                                                 "Other modes",
#                                                 "No commute"))
```
...
\
\
Those who selected "Usually the same location (oustide home)" or "Workplace regularly varies (different offices or jobsites)" were included to understand commute modes. 
```{r}
...
freq(psurvey_17_simp$transit_mode)
```
\
\
...
### Race
```{r}
race_com_cnt_2017 <-count_prep_fx(psurvey_17_simp, 'race_eth_broad', 'transit_mode')
...
```{r}
race_com_2017_noveh_cnt <- count_prep_fx(race_com_2017_noveh,'race_eth_broad', 'hhsize')

share_plot(race_com_2017_noveh_cnt, 'race_eth_broad', 'hhsize', 'Race', 'Household size and Race: 2017, No household vehicles')
```
\

```{r}
race_com_2017_noveh$hhadults_veh_chr <- as.character(race_com_2017_noveh$numadults)

race_com_2017_noveh_cnt <- count_prep_fx(race_com_2017_noveh, 'race_eth_broad', 'hhadults_veh_chr')

share_plot(race_com_2017_noveh_cnt, 'race_eth_broad', 'hhadults_veh_chr', 'Race', 'Household adults and Race: 2017, No household vehicles')
```
\
...
## Auto Ownership
### Household persons and number of vehicles
1. **hhsize/vehicle_count** = Household size
2. **numadults/vehicle_count** = Number of adults (18+)
3. **numworkers/vehicle_count** = Number of workers (number of household members with 1+ jobs)
\
...
#### 1. Household size
```{r}
hist(race_com_2017_numveh$hhsz_veh, 
     main="Household size and vehicle ownership",
     xlab="People : Vehicles",
     breaks=23,
     ylim=c(0,3000))

race_com_2017_noveh_cnt <- count_prep_fx(race_com_2017_numveh,'race_eth_broad', 'hhsz_veh_grp')

share_plot(race_com_2017_noveh_cnt, 'race_eth_broad', 'hhsz_veh_grp', 'Race', 'Household vehicles and Race: 2017, Household persons/veh')
```
\
\
...
#### 2. Number of adults
```{r}
hist(race_com_2017_numveh$hhadults_veh, 
     main="Household adults and vehicle ownership",
     xlab="Adults : Vehicles",
     breaks=23,
     ylim=c(0,3000))

race_com_2017_noveh_cnt <- count_prep_fx(race_com_2017_numveh,'race_eth_broad', 'hhadults_veh_grp')

share_plot(race_com_2017_noveh_cnt, 'race_eth_broad', 'hhadults_veh_grp', 'Race', 'Household vehicles and Race: 2017, Household workers/veh')
```
\
\
