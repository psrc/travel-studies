...
output_path <- "C:/Coding/CURRENT_REPOS_GITHUB/travel-studies/2021/analysis_in_progress/deliveries"
...
if(file.exists(paste0(output_path, "/delivery_by_freq_food2.csv"))){
  
  delivery_by_freq_food2<-read.csv(paste0(output_path, '/delivery_by_freq_food2.csv'))
}else{
...
delivery_by_freq_food2_trend<-interactive_column_chart(t=delivery_by_freq_food2, x='delivery_food_all', y='share', fill='survey', moe='share_moe', est='percent', color = "pgnobgy_5", title="Share of Households Receiving Food Deliveries")

saveWidget(delivery_by_freq_food2, 
           file=paste0(output_path,'/trend_visuals/delivery_by_freq_food2.html'))

#delivery_by_freq_food2_trend_int<-create_column_chart(t=delivery_by_freq_food2_no_drive, x='mode_simple', y='share', f='year', moe='share_moe', est='percent', color = "pgnobgy_5", title="Non-Driving Trip Mode Shares", interactive='yes')

#%>%layout(xaxis=list(title='Trip Mode'),yaxis=list(title='Percent of All Trips') )

#saveWidget(delivery_by_freq_food2_trend_int, file=paste0(output_path, '/delivery_by_freq_food2.html'))

addWorksheet(wb = wb, sheetName = 'delivery_by_freq_food2', gridLines = FALSE)
writeDataTable(wb = wb, sheet = 'delivery_by_freq_food2', x = delivery_by_freq_food2)
```
## Including Plots
