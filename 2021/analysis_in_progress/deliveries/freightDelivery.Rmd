...
hh_data <- c('hhsize', 'lifecycle', 'hhincome_broad', 'hhincome_detailed', 'hhincome_dichot', 'race_category_alone_multi', 'race_eth_broad', 'race_eth_poc', 'race_eth_apoc', 'hh_race_category', 'seattle_home', 'final_home_rgcnum', 'final_home_uvnum', 'final_home_is_rgc','pernum')
    ...
    mutate(delivery_work_all= case_when((pernum==1 & is.na(delivery_work_freq) & is.na(deliver_work)) ~ 'No HH Response',
     ...
     mutate(lifecycle= case_when(lifecycle == "Household size > 1, Householder age 65+" | 
                                  lifecycle == "Household size = 1, Householder age 65+"  ~ '65 years or older', 
                                   ...
                                   lifecycle == "Household size = 1, Householder under age 35" ~ '35-64 years, no kids',
...
food_c <-create_column_chart(t=food_freq_separate, x='delivery_food_all', y='share', f='survey', moe='share_moe', 
  ...
  xlab(as.character('Food')) + ylab('Share of Households')+
...
food_column<- create_column_chart(t= food_freq_separate,
...
pkgs_column<- create_column_chart(t= pkgs_freq_separate,
...
grocery_column<- create_column_chart(t= grocery_freq_separate,
...
work_column<- create_column_chart(t= work_freq_separate,
...
food_income_column<- create_column_chart(t= all_food_income_dichot_17_21,
...
work_income_17_dichot <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
...
work_income_19_dichot <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('hhincome_dichot', 'delivery_work_all')) %>% 
  filter(hhincome_dichot != 'Total')%>%
  filter(hhincome_dichot != 'Prefer not to answer') %>%
  filter(delivery_work_all != 'Total')%>%
...
work_income_1719_dichot <- hhts_count(joined_df_deliveries_1719, spec_wgt = 'day_weight_2017_2019_v2021', 
                    group_vars = c('hhincome_dichot', 'delivery_work_all')) %>% 
  filter(hhincome_dichot != 'Total')%>%
  filter(hhincome_dichot != 'Prefer not to answer') %>%
  filter(delivery_work_all != 'Total')%>%
...
work_income_21_dichot <- hhts_count(joined_df_deliveries_21, spec_wgt = 'hh_weight_2021_ABS', 
                    group_vars = c('hhincome_dichot', 'delivery_work_all')) %>% 
  filter(hhincome_dichot != 'Total')%>%
  filter(hhincome_dichot != 'Prefer not to answer') %>%
  filter(delivery_work_all != 'Total')%>%
...
work_test<- create_column_chart(t= all_work_income_dichot_17_21,
                                                  x="hhincome_dichot", y="share",
                                                  f="survey",
                                            moe = "share_moe",
                                                  color="psrc_pairs",
...
food_lifecycle_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
  ...
  mutate(lifecycle = fct_relevel(lifecycle,
            "Household has kids", "35-64 years, no kids", "65 years or older"))
...
lifecycles_test<- create_column_chart(t= all_food_lifecycle_17_21,
                                                  x="lifecycle", y="share",
                                                  f="survey",
                                            moe = "share_moe",
                                                  color="psrc_pairs",
...
grocery_lifecycle_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('delivery_grocery_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_grocery_all != 'Total')%>%
  filter(delivery_grocery_all != 'No Delivery')%>%
  mutate(lifecycle = fct_relevel(lifecycle,
...
grocery_lifecycle_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('delivery_grocery_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_grocery_all != 'Total')%>%
  filter(delivery_grocery_all != 'No Delivery')

grocery_lifecycle_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
...
lifecycles_test<- create_column_chart(t= all_grocery_lifecycle_17_21,
                                                  x="lifecycle", y="share",
                                                  f="survey",
                                            moe = "share_moe",
                                                  color="psrc_pairs",
...
pkgs_lifecycle_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('delivery_pkgs_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_pkgs_all != 'Total')%>%
  filter(delivery_pkgs_all != 'No Delivery')%>%
  mutate(lifecycle = fct_relevel(lifecycle,
...
pkgs_lifecycle_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('delivery_pkgs_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_pkgs_all != 'Total')%>%
  filter(delivery_pkgs_all != 'No Delivery')

pkgs_lifecycle_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
...
lifecycles_pkgs<- create_column_chart(t= all_pkgs_lifecycle_17_21,
                                                  x="lifecycle", y="share",
                                                  f="survey",
                                            moe = "share_moe",
                                                  color="psrc_pairs",
...
work_lifecycle_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('delivery_work_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_work_all != 'Total')%>%
  filter(delivery_work_all != 'No Delivery')%>%
  mutate(lifecycle = fct_relevel(lifecycle,
...
work_lifecycle_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('delivery_work_all', 'lifecycle')) %>% 
  filter(lifecycle != 'Total')%>%
  filter(delivery_work_all != 'Total')%>%
  filter(delivery_work_all != 'No Delivery')

work_lifecycle_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
...
lifecycles_work<- create_column_chart(t= all_work_lifecycle_17_21,
                                                  x="lifecycle", y="share",
                                                  f="survey",
                                            moe = "share_moe",
                                                  color="psrc_pairs",
...
food_lifecycle_21$delivery_type = c("food/meal", "food/meal", "food/meal")

grocery_lifecycle_21$delivery_type = c("grocery", "grocery", "grocery")

pkgs_lifecycle_21$delivery_type = c("packages", "packages", "packages")

work_lifecycle_21$delivery_type = c("work/service", "work/service", "work/service")
...
```{r, hhsize and food deliveries, produced grocery charrt with this but had to revert}
# hh variables: hhsize'

food_hh_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('hhsize', 'delivery_food_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_food_all != 'Total')%>%
  filter(delivery_food_all != 'No Delivery')

food_hh_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('hhsize', 'delivery_food_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_food_all != 'Total')%>%
  filter(delivery_food_all != 'No Delivery')

food_hh_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
                    group_vars = c('hhsize', 'delivery_food_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_food_all != 'Total')%>%
  filter(delivery_food_all != 'No Delivery')
...
food_hhsize_plot<- create_column_chart(t= all_food_hh_17_21,
                                                  x="hhsize", y="share",
                                                  f="survey",
                                                  moe = "share_moe",
                                                  color="psrc_pairs",
...
pkgs_hh_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('hhsize', 'delivery_pkgs_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_pkgs_all != 'Total')%>%
  filter(delivery_pkgs_all != 'No Delivery')

pkgs_hh_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('hhsize', 'delivery_pkgs_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_pkgs_all != 'Total')%>%
  filter(delivery_pkgs_all != 'No Delivery')

pkgs_hh_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
                    group_vars = c('hhsize', 'delivery_pkgs_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_pkgs_all != 'Total')%>%
  filter(delivery_pkgs_all != 'No Delivery')
...
pkgs_hhsize_plot<- create_column_chart(t= all_pkgs_hh_17_21,
                                                  x="hhsize", y="share",
                                                  f="survey",
                                                  moe = "share_moe",
                                                  color="psrc_pairs",
...
work_hh_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017_v2021', 
                    group_vars = c('hhsize', 'delivery_work_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_work_all != 'Total')%>%
  filter(delivery_work_all != 'No Delivery')

work_hh_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019_v2021', 
                    group_vars = c('hhsize', 'delivery_work_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_work_all != 'Total')%>%
  filter(delivery_work_all != 'No Delivery')

work_hh_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_weight_2021_ABS_panel_adult', 
                    group_vars = c('hhsize', 'delivery_work_all')) %>% 
  filter(hhsize != 'Total')%>%
  filter(delivery_work_all != 'Total')%>%
  filter(delivery_work_all != 'No Delivery')
...
work_hhsize_plot<- create_column_chart(t= all_work_hh_17_21,
                                                  x="hhsize", y="share",
                                                  f="survey",
                                                  moe = "share_moe",
                                                  color="psrc_pairs",
