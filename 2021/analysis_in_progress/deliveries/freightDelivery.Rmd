...
```{r, getting survey data with pre-identified variables}
...
```{r, generating datasets}

# transform data with visualization function code block 
...
# in previous versions of the code, two datasets/views (days and household views) were merged, so code further in script references
# "joined_df_deliveries_year"
...
# In 2021, this represents the number of adults because we did not observe children's days (this was done in 2017 & 2019 only).
```{r, population data sums based off of assigned table weights}
...
# not fully sure what the next three chunks are doing and if additional calculations needs to be conducted for the combo 2017/2019 data
```{r, 2017/2019 dataset amendments}
# Based on the "day weight for 2017_2019" value, the sum should be halved.
...
```{r}
# remove duplicate household responses
...
# checking the sums with various weights

sum(joined_df_deliveries_17$day_weight_2017_v2021)
sum(joined_df_deliveries_19$day_weight_2019_v2021)
sum(joined_df_deliveries_21$hh_weight_2021_ABS) # this is the 2021 weight that will be used to calculate analysis
sum(joined_df_deliveries_21$person_weight_2021_ABS_panel_adult)
