...
hh_data <- c('lifecycle', 'hhincome_broad', 'hhincome_detailed', 'hhincome_dichot', 'race_category_alone_multi', 'race_eth_broad', 'race_eth_poc', 'race_eth_apoc', 'hh_race_category', 'seattle_home', 'final_home_rgcnum', 'final_home_uvnum', 'final_home_is_rgc','pernum', 'hhsize', 'numworkers', 'numtrips')
...
?merge
all_deliveries_17_21 <- merge(joined_df_deliveries_17, joined_df_deliveries_19, joined_df_deliveries_21, by= c('survey', 'household_id'))
  #mutate('delivery_food_all' = ifelse(delivery_pkgs_all=='Delivery Received', 1, 0)) %>%
  #mutate('delivery_pkgs_all' = ifelse(delivery_pkgs_all=='Delivery Received', 1, 0)) %>%
  #mutate('delivery_grocery_all' = ifelse(delivery_pkgs_all=='Delivery Received', 1, 0)) %>%
  #mutate('delivery_work_all' = ifelse(delivery_pkgs_all=='Delivery Received', 1, 0)))
```
...
# plots
# using delivery_trips_modeling_meg.Rmd

# crosstabs for lifecycle and food delivery by share and count (across years)

food_hhincome_broad_17 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'day_weight_2017', 
                    group_vars = c('delivery_food_all', 'hhincome_broad')) %>% 
  filter(hhincome_broad != 'Total')

food_hhincome_broad_19 <- hhts_count(joined_df_deliveries_19, spec_wgt = 'day_weight_2019', 
                    group_vars = c('delivery_food_all', 'hhincome_broad')) %>% 
  filter(hhincome_broad != 'Total')

food_hhincome_broad_21 <- hhts_count(joined_df_deliveries_21, spec_wgt = 'person_adult_weight_2021', 
                    group_vars = c('delivery_food_all', 'hhincome_broad')) %>% 
  filter(hhincome_broad != 'Total')

# merge dfs for 2017, 2017/2019, and 2021 - alternate approach for crosstabs
all_food_hhincome_broad_17_21 <- bind_rows(food_hhincome_broad_17, food_hhincome_broad_19, food_hhincome_broad_21) %>%
  mutate(period = as.factor(survey)) %>%
  filter(delivery_food_all != 'No Delivery')

income_broad_facet <- create_facet_bar_chart(t= all_food_hhincome_broad_17_21,
                                                  w.x="survey", w.y="share",
                                                  f="survey", g="hhincome_broad",
                                                  w.color="psrc_light",
                                                  est.type ="percent",
                                                  w.interactive="no",
                                                  w.dec=2,
                                                  w.scales="fixed",
                                                  w.facet=6,
                                                  w.title="Home Deliveries or Services by Income (Broad)",
                                                  w.sub.title="Share of Households on Average Weekday")+ 
