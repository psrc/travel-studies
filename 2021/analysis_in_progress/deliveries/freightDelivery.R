...
smp_commute_fx <- function(data, year) {
  ...
  temp_table <- data %>%
    mutate(delivery_food_freq = case_when(delivery_food_freq == "0 (none)" ~ '0',
                                          delivery_food_freq == "5 or more" ~ "5+",
                                          delivery_food_freq == "Missing: Skip Logic" |
                                            delivery_food_freq == "NA" ~ "NA",
                                          TRUE ~ delivery_food_freq)) %>%
    mutate(delivery_grocery_freq = case_when(delivery_grocery_freq == "0 (none)" ~ '0',
                                             delivery_grocery_freq == "5 or more" ~ "5+",
                                             delivery_grocery_freq == "Missing: Skip Logic" |
                                               delivery_grocery_freq == "NA" ~ "NA",
                                             TRUE ~ delivery_grocery_freq)) %>%
    mutate(delivery_pkgs_freq = case_when(delivery_pkgs_freq == "0 (none)" ~ '0',
                                          delivery_pkgs_freq == "5 or more" ~ "5+",
                                          delivery_pkgs_freq == "Missing: Skip Logic" |
                                            delivery_pkgs_freq == "NA" ~ "NA",
                                          TRUE ~ delivery_pkgs_freq)) %>%
    mutate(delivery_work_freq = case_when(delivery_work_freq == "0 (none)" ~ '0',
                                          delivery_work_freq == "5 or more" ~ "5+",
                                          delivery_work_freq == "Missing: Skip Logic" |
                                            delivery_work_freq == "NA" ~ "NA",
                                          TRUE ~ delivery_work_freq)) %>%
    mutate(delivery_food_freq = factor(case_when(as.character(delivery_food_freq) %in% c("Missing: Skip Logic","NA") ~ "NA",
        !is.na(delivery_food_freq) ~ as.character(delivery_food_freq)),
        levels=c("0", "1", "2", "3", "4", "5+", "NA")))
}

smp_commute_fx_hh <- function(data, year) {
  ## rewriting labels of responses to be more concise
  temp_table <- data %>%
...
hsurvey_17 <- get_hhts(survey = survey_c$survey, 
                       level = "h", 
                       vars = c(hh_data))
...
hsurvey_19 <- get_hhts(survey = survey_d$survey, 
                       level = "h", 
                       vars = c(hh_data))
...
hsurvey_1719 <- get_hhts(survey = survey_a$survey, 
                       level = "h", 
                       vars = c(hh_data))
...
hsurvey_21 <- get_hhts(survey = survey_b$survey, 
                       level = "h", 
                       vars = c(hh_data))

# transform data

delivery_17 <- smp_commute_fx(dsurvey_17, '2017')

delivery_17b <- smp_commute_fx_hh(hsurvey_17, '2017')

delivery_19 <- smp_commute_fx(dsurvey_19, '2019')

delivery_19b <- smp_commute_fx_hh(hsurvey_19, '2019')

delivery_1719 <- smp_commute_fx(dsurvey_1719, '2017/2019')

delivery_1719b <- smp_commute_fx_hh(hsurvey_1719, '2017/2019')

delivery_21 <- smp_commute_fx(dsurvey_21, '2021')

delivery_21b <- smp_commute_fx_hh(hsurvey_21, '2021')
...
count_food_17 <- hhts_count(joined_df_deliveries_17, group_vars = "delivery_food_freq",
  ...
  #filter(joined_df_deliveries != "Total")
...
test2 <- hhts_count(joined_df_deliveries_17, spec_wgt = 'hh_weight_2017_v2021', 
