...
## Changes over time
To test if the number of zero-vehicle households in RGCs increased in 2021?
```{r}
...
```
As a basic test, there is no overlap in the 90 percent confidence intervals of the two estimates (2017/19 and 2021), so we can conclude that the values are different and that there was a change over time.
\
This difference is confirmed with the z-score, `r z_0veh`
\
\

To test if the number of fewer vehicles than workers households decreased in 2021?
```{r}
z_lessveh <- z_score(xy17[7], xy21[7])
```
Following a similar process as before, there is no overlap between the two survey years indicating that there was a change over time.
\
This difference is confirmed with the z-score, `r z_lessveh`
\
\
