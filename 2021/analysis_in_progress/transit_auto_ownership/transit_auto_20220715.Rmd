...
\
\
```{r, CSV output all workers, include=FALSE, eval=FALSE}
# generate data
...
```
\
For households with more vehicles than workers, the difference between Black or African American and all other racial groups  is larger in 2017/19 (especially compared to White only). In 2021 when the MOEs are overalpping except for White only. In 2021 White only households decreased in this category and increased in the one vehicle per worker households. 
\
\
...
```
\
Non-POC households reported more vehicles than workers in 2017/19 compared to POC, but in 2021, the difference is smaller with overlapping MOEs. Because the proportion of POC households with more vehicles than workers didn't chane too much, the difference seems to be for Non-POC.
\
...
```
\
In 2021, there were more Asian POC households with fewer vehicles than workers compared to Non-POC, but the difference with Non-Asian POC is difficult to determine because of MOEs.
\
\
...
```
\
Zero-vehicle households tend to make less money than households with at least one vehicle, especially in 2021. The overlapping MOEs make it difficult to determine any other changes between years, but the trends seem similar.  
\
\
...
**Would it be better to create 2 groups instead of 3?**
\
\
