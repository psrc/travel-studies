...
# tracts_layer<- st_transform(tracts_20, crs=wgs_84)
...
map.lon<- -122.257
map.zoom<- 8
pal <- leaflet::colorNumeric(palette="Blues", domain = veh_num_tracts$veh_by_tract)
veh_registration_map<- leaflet()%>%
     addProviderTiles(providers$CartoDB.Positron) %>%
  addLayersControl(baseGroups = c("Base Map"),
                      overlayGroups = c("Travel Survey Origins", "Seattle Sectors"),
                      options = layersControlOptions(collapsed = TRUE))%>%
  addPolygons(data=veh_num_tracts,
               fillColor = pal(veh_num_tracts$veh_by_tract),
               weight = 4,
               opacity = 0.1,
               color = "#91268F",
               dashArray = "4",
    ...
    leaflet::addLegend(pal = pal,
                       values = veh_num_tracts$veh_by_tract,
