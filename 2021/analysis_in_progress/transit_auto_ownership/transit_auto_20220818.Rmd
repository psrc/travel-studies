...
# test_data <- get_hhts("2017_2019", "p", c("commute_mode", "race_eth_broad"))
# test_data <- get_hhts("2021", "p", all_vars_p)
# 
# c <- hhts_count(test_data, 
#                 group_vars="race_eth_broad")
# d <- hhts_count(test_data, 
#                 group_vars="race_eth_broad",
...
```{r}
# data set up ----
veh_stats <- matrix(c(6.9,29.2,37.3,26.2,
                      10.5,34.8,35.3,19.4,
                      4.7,25.7,40.3,29.3,
                      5.2,27.2,39.4,28.2,
                      4.7,25.2,39.7,30.4),
                      ncol=5, byrow=FALSE)
veh_stats <- veh_stats*.01
colnames(veh_stats) <- c("Washington", "King", "Kitsap", "Pierce", "Snohomish")
rownames(veh_stats) <- c("No vehicle available", "1 vehicle available", "2 vehicles available", "3+ vehicles available")
...
nat_2 <- 0.371
nat_3 <- 0.219
nat_veh_num <- c(0.085,0.325,0.371,0.219)
...
# plotting ----
# summary(psrc_colors)
veh_stats_plot3 <- create_bar_chart(t=census_data_df, w.x="Vehicles",w.y="Households",
                                    f="Geography",
                                    # h.ref = c(nat_no,nat_1,nat_2,nat_3),
                                    h.ref = nat_veh_num,
                                    h.ref.nm = c("National (0 vehicles)",
                                                 "National (1 vehicle)",
                                                 "National (2 vehicles)",
                                                 "National (3+ vehicles)"),
                                    h.ref.cl = c(psrc_colors$psrc_light[1:4]),
                                    w.title="Household Vehicle Ownership",
                                    w.sub.title="Latest ACS 5-Year Estimates | Data Profiles/Housing Characteristics",
                                    w.interactive="yes")
...
# Connecting to ElmerGeo trial #1 ----
...
# 
# Connecting to ElmerGeo trial #2 ----
...
tbl_nm <- "T:/2022August/Mary/HHTS/geo_data/tracts_2020.shp"
tracts_layer <- st_read(tbl_nm)
tracts_layer <- st_transform(tracts_layer, crs=wgs_84)

tbl_nm <- "T:/2022August/Mary/HHTS/geo_data/ElmerGeo_hct_stops.shp"
stops_layer <- st_read(tbl_nm)
hct_stops <- st_transform(stops_layer, crs=wgs_84)

tbl_nm <- "T:/2022August/Mary/HHTS/geo_data/ElmerGeo_hct_trans.shp"
trans_layer <- st_read(tbl_nm)
hct_trans <- st_transform(trans_layer, crs=wgs_84)

tbl_nm <- "T:/2022August/Mary/HHTS/geo_data/ElmerGeo_urban_growth_area.shp"
uga_layer <- st_read(tbl_nm)
urbangrowtharea <- st_transform(uga_layer, crs=wgs_84)
...
vbp_pal <- leaflet::colorNumeric(palette="Blues",
                                 veh_num_tracts_pop$veh_by_pop_tract)

stops_pal <- leaflet::colorNumeric(palette="Red",
                                   hct_stops$brt_tod)

trans_pal <- leaflet::colorNumeric(palette="Orange",
                                   hct_trans$new_facili)

uga_pal <- leaflet::colorFactor(palette=c("Pink"),
                                urbangrowtharea$objectid)
...
veh_registration_map <- leaflet() %>%
  ...
  addCircles(data=hct_stops,
              color = stops_pal(hct_stops$brt_tod),
              opacity = 0.7,
              group = "HCT stops",
              label = ~brt_tod) %>%
  addPolygons(data=hct_trans,
              color = ~trans_pal(new_facili),
              stroke=TRUE,
              opacity = 0.5,
              group = "HCT transit") %>%
  addPolygons(data=urbangrowtharea,
              fillColor = uga_pal(urbangrowtharea$objectid),
              stroke=FALSE, 
              smoothFactor = 0.2,
              fillOpacity = 0.6,
              group = "Urban growth area",
              label = ~objectid)%>%
  # legends
  ...
  hideGroup("HCT stops") %>%
  hideGroup("HCT transit") %>%
  hideGroup("Urban growth area") %>% 
  #set view extent
