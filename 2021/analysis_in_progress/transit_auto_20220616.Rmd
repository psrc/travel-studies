...
psurvey_19_simp <- psurvey_19 %>%
  ...
  mutate(commute_mode = factor(case_when(commute_mode=="Bus (public transit)" |
                                         ...
                                         commute_mode=="Drive alone" ~ "3-Drive alone",
                                         TRUE ~ "4-Other modes"))) %>% 
  mutate(final_home_rgcnum = factor(case_when(final_home_rgcnum=="Not RCG" ~ "Not RGC",
                                              TRUE ~ "RGC")))
```
...
psurvey_1719_simp <- psurvey_1719 %>%
  ...
  mutate(commute_mode = factor(case_when(commute_mode=="Bus (public transit)" |
                                         ...
                                         commute_mode=="Drive alone" ~ "3-Drive alone",
                                         TRUE ~ "4-Other modes"))) %>% 
  mutate(final_home_rgcnum = factor(case_when(final_home_rgcnum=="Not RCG" ~ "Not RCG",
                                              TRUE ~ "RCG")))
```
...
```{r}
race_com_2017_noveh <- psurvey_17_simp_2grp %>% 
  filter(vehicle_count=="0 (no vehicles)")

race_com_2017_noveh_cnt <- count_prep_fx(race_com_2017_noveh,'race_eth_broad', 'commute_mode')

share_plot(race_com_2017_noveh_cnt, 'race_eth_broad', 'commute_mode', 'Race', 'Commute Mode and Race: 2017, No household vehicles')
```
\
...
all_transit_19 <- count_prep_fx(psurvey_19_simp,'race_eth_broad', 'commute_mode')
all_transit_1719 <- count_prep_fx(psurvey_1719_simp,'race_eth_broad', 'commute_mode')
all_transit_21 <- count_prep_fx(psurvey_21_simp,'race_eth_broad', 'commute_mode')
...
```{r}
race_com_2017_numveh <- psurvey_17_simp_2grp %>% 
  filter(vehicle_count!="0 (no vehicles)") %>% 
