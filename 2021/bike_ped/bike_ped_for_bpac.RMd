...
trips_by_mode_21<-hhts_count(Walk_bike_data_21_no_miss_acc, group_vars='mode_acc', spec_wgt='trip_weight_2021_ABS_Panel_adult')%>%filter(mode_acc!='Total')
...
p<-create_bar_chart(t=trips_purp , w.x='simple_purpose', w.y='share', f='survey', w.moe='share_moe', est.type='percent')+theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+
...
```{r}
  ...
  wbfreq_summs_2021 <- hhts_count(Walk_Walk_freq_data_21,
    ...
    mutate(survey = str_replace_all(survey, "_", "/"))
      ...
      p1<-create_bar_chart(t=wbfreq_summs_long , w.x='mode_freq_3', w.y='share', f='survey', w.moe='share_moe', est.type='percent', w.color = 'psrc_dark', w.interactive=TRUE)+
...
if(cat_name=='hhincome_100_f'){
 ...
 wbfreq_summs_2021$hhincome_100_f=factor(wbfreq_summs_2021$hhincome_100_f, levels=c("Prefer not to answer",  "$100,000 or more","$75,000-$99,999", "$50,000-$74,999" ,"$25,000-$49,999" , "Under $25,000"  ))
  ...
  #transit_summs_long <- factor_variable_column(names(g), transit_summs_long)
   ...
   p0<-create_bar_chart(t=wbfreq_summs_2021 , w.x=cat_name, w.y='share', f='Walks_sometimes', w.moe='share_moe', est.type='percent', w.color = 'psrc_dark', w.interactive=TRUE)+
  xlab(as.character(g[cat_name])) + ylab('Share who Walk Sometimes')+theme(axis.text.x = element_text(size=14,color="#4C4C4C")) +theme(axis.title.y = element_text(size=20,color="#4C4C4C"))+coord_flip()
  print(p0)

  p1<-create_bar_chart(t=wbfreq_summs_long , w.x=cat_name, w.y='share', f='survey', w.moe='share_moe', est.type='percent', w.color = 'psrc_dark', w.interactive=TRUE)+
  xlab(as.character(g[cat_name])) + ylab('Share who Walk Sometimes')+theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+ theme(axis.title.x = element_text(size=20,color="#4C4C4C"))+theme(axis.title.y = element_text(size=20,color="#4C4C4C"))+coord_flip()
  print(p1)
...
if(cat_name=='hhincome_100_f'){
 ...
 wbfreq_summs_2021$hhincome_100_f=factor(wbfreq_summs_2021$hhincome_100_f, levels=c("Prefer not to answer",  "$100,000 or more","$75,000-$99,999", "$50,000-$74,999" ,"$25,000-$49,999" , "Under $25,000"  ))
  ...
  xlab(as.character(g[cat_name])) + ylab('Share who Bike Sometimes')+theme(axis.text.x = element_text(size=14,color="#4C4C4C")) +theme(axis.title.y = element_text(size=20,color="#4C4C4C"))+coord_flip()
  print(p0)
  ...
  xlab(as.character(g[cat_name])) + ylab('Share who Bike Sometimes')+theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+ theme(axis.title.x = element_text(size=20,color="#4C4C4C"))+theme(axis.title.y = element_text(size=20,color="#4C4C4C"))+coord_flip()
  print(p1)
...
ggplot(all_Walks, aes(x=trip_path_distance, color=survey)) + geom_histogram(position=position_dodge(0.3), aes(weight=trip_wt, fill=survey), alpha=0.5, bins=30)+theme(axis.text.x = element_text(size=14,color="#4C4C4C"))+
