...
```{r}
deliv_model<-glm(formula= delivery_pkg~hhincome_detailed+lifecycle+survey.x+numworkers+shop_trips+final_home_is_rgc+hh_race_apoc+numchildren+numadults+nonwhite+poor_english+rent+severe_cost_burdened+poverty_200+prox_high_inc+transit_qt_mile+dist_super+ln_jobs_auto_30,family=binomial(), data=hh_shop_tract)
summary(deliv_model)
...
```{r}
deliv_model_sig<-glm(formula= delivery_pkg~hhincome_detailed+lifecycle+survey.x+numworkers+poor_english+prox_high_inc+transit_qt_mile+shop_trips,family=binomial(), data=hh_shop_tract)
