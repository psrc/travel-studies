...
```{r}
trip_vars<-c('mode_1', 'mode_simple','dest_purpose_cat', 'origin_purpose_cat', 'trip_path_distance', 'household_id', 'age_category', 'daynum' )
...
```{r}
trip_data_17<- get_hhts("2017", "t", vars=trip_vars)%>%filter(age_category!='Under 18 years')%>%select(-trip_weight_2017)
trip_data_19<- get_hhts("2019", "t", vars=trip_vars)%>%filter(age_category!='Under 18 years')%>%select(-trip_weight_2019)
trip_data_21<- get_hhts("2021", "t", vars=trip_vars)%>%filter(age_category!='Under 18 years')%>%select(-c(trip_adult_weight_2021, trip_respondent_weight_2021))
trip_data_17_19_21<-rbind(trip_data_17, trip_data_19, trip_data_21)

household_data_17<- get_hhts("2017", "h", vars=household_vars)%>%select(-hh_weight_2017)
household_data_19<- get_hhts("2019", "h", vars=household_vars)%>%select(-hh_weight_2019)
household_data_21<- get_hhts("2021", "h", vars=household_vars)%>%select(-hh_weight_2021)
...
```{r}
shop_model<-polr(formula= shop_trips~hhsize+hhincome_detailed+survey, data=household_data_17_19_21_shop)
summary(shop_model)
```
```{r}
+ final_home_is_rgc+hhsize+lifecycle
```
  ...
  temp_table <- data %>%
    ...
    mutate(delivery_work_all= case_when((pernum==1 & is.na(delivery_work_freq) & is.na(deliver_work)) ~ 'No HH Response',
                                        ...
                                        delivery_work_freq == "0 (none)"  ~ 'No Delivery',
                                         TRUE ~ 'Delivery Received'))
  temp_table<-temp_table%>%filter(pernum==1)%>% dplyr::select(survey,household_id,daynum, deliver_package, deliver_grocery, deliver_food )
  temp_table
}
```
...
```{r}
shop_model<-polr(formula= shop_trips~hhsize+hhincome_detailed+survey.x+deliver_package+deliver_grocery+NoVehicles+final_home_is_rgc, data=household_data_17_19_21_shop_del)
summary(shop_model)
```
