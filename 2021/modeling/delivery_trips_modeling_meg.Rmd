...
deliv_model<-glm(formula= delivery_pkg~hhincome_detailed2+lifecycle2+final_home_is_rgc+hh_race_apoc+poor_english+severe_cost_burdened+poverty_200+prox_high_inc+transit_qt_mile+dist_super+ln_jobs_auto_30+rent_own+NoVehicles+broadband2,family=binomial(), data=hh_shop_tract1)
summary(deliv_model)
...
deliv_model_lifecycle<-glm(formula= delivery_pkg~lifecycle2, family=binomial(), data=hh_shop_tract)
...
deliv_model_numworkers<-glm(formula= delivery_pkg~numworkers, family=binomial(), data=hh_shop_tract)
...
deliv_model_shop<-glm(formula= delivery_pkg~shop_trips, family=binomial(), data=hh_shop_tract)
...
deliv_model_rgc<-glm(formula= delivery_pkg~final_home_is_rgc, family=binomial(), data=hh_shop_tract)
...
deliv_model_race<-glm(formula= delivery_pkg~hh_race_apoc, family=binomial(), data=hh_shop_tract3)
...
```{r, broadband}
deliv_model_sig<-glm(formula= delivery_pkg~+broadband2,family=binomial(), data=hh_shop_tract)
...
```{r, mobile}
deliv_model_sig<-glm(formula= delivery_pkg~mobile_device,family=binomial(), data=hh_shop_tract)
summary(deliv_model_sig)
plot_summs(deliv_model_sig, scale=TRUE)
...
```{r, rent own}
deliv_model_sig<-glm(formula= delivery_pkg~rent_own,family=binomial(), data=hh_shop_tract)
summary(deliv_model_sig)
plot_summs(deliv_model_sig, scale=TRUE)
...
```{r, vehicles}
deliv_model_sig<-glm(formula= delivery_pkg~NoVehicles,family=binomial(), data=hh_shop_tract)
summary(deliv_model_sig)
plot_summs(deliv_model_sig, scale=TRUE)
