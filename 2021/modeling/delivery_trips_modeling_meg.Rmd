...
household_data<-household_data%>%
  ...
  mutate(broadband2=case_when(broadband == 'Broadband service is available where I live, but I DO NOT have broadband service at my residence'| 
           ...
           mutate(rent_own= case_when(rent_own == 'Own/paying mortgage' ~ 'Own',
                                      rent_own == 'Rent' ~ 'Rent')) %>%
  filter(rent_own != c('Provided by family, relative, or friend without payment or rent' , 'Provided by job or military', 'Prefer not to answer'))
...
count(hh_shop_tract, hhincome_broad)
unique(hh_shop_tract$hhincome_broad)

hh_shop_tract2 <- hh_shop_tract %>%
  filter(hhincome_broad != c("Prefer not to answer"))

deliv_model_income<-glm(formula= delivery_pkg~hhincome_broad, family=binomial(), data=hh_shop_tract2)
