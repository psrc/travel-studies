...
household_vars<-c('vehicle_count',  "hhincome_broad",  'hhincome_detailed', 'final_home_is_rgc', 'hhsize', 'household_id', 'lifecycle', 'sample_source', 'hhgroup', 'final_home_tract10', 'hh_race_apoc', 'numworkers', 'numchildren', 'numadults', 'broadband', 'mobile_device', 'res_factors_transit', 'res_factors_walk')
```
...
household_data<-household_data%>%mutate(NoVehicles=ifelse(vehicle_count=='0 (no vehicles)', 'No Vehicles', "Has Vehicles")) %>%mutate(hhincome_100= case_when(
  ...
  mutate(broadband=case_when(broadband == 'Broadband service is available where I live, but I DO NOT have broadband service at my residence'| 
                               ...
                               broadband == 'Don’t know' ~ 'No',
                             broadband == 'Yes, my current residence has broadband service' ~ 'Yes'))
...
```{r}
# packages
deliv_model<-glm(formula= delivery_pkg~hhincome_detailed+lifecycle+survey.x+numworkers+shop_trips+final_home_is_rgc+hh_race_apoc+numchildren+numadults+nonwhite+poor_english+rent+severe_cost_burdened+poverty_200+prox_high_inc+transit_qt_mile+dist_super+ln_jobs_auto_30+broadband+mobile_device+
                   vehicle_count+final_home_is_rgc+hhsize+shop_trips,family=binomial(), data=hh_shop_tract)
summary(deliv_model)
...
```{r}
deliv_model_sig<-glm(formula= delivery_pkg~hhincome_detailed+lifecycle+survey.x+numworkers+poor_english+prox_high_inc+transit_qt_mile+shop_trips+
                       broadband+hhsize+dist_super+NoVehicles,family=binomial(), data=hh_shop_tract)
