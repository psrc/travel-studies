...
source("C:/Users/JLin/OneDrive - Puget Sound Regional Council/Documents/psrc_work_files/data_science/vmt_research/hhts_var_function.R")
```
...
vmt_per %>% 
  ...
  mutate(vmt_day_mean = vmt_day_mean/100,
...
vmt_per %>% 
  ...
  e_tooltip(formatter = htmlwidgets::JS("
      function(params){
        return('county: ' + params.value[0] + '<br />vmt mean: ' + params.value[1])
      }
    "))

# vmt in RGCs
test <-vmt_per %>% 
  hhts_mean(stat_var="vmt_day", group_vars=c("urban_metro","final_home_rgcnum")) %>%
  mutate(lower = vmt_day_mean-vmt_day_mean_moe,
         upper = vmt_day_mean+vmt_day_mean_moe) %>%
   filter(final_home_rgcnum!="Total")
test %>% 
  arrange(vmt_day_mean) %>%
  e_charts(final_home_rgcnum)  |> 
  e_bar(vmt_day_mean, name = "mean of vmt", 
        bind = urban_metro, # include column urban_metro in graph
        colorBy=urban_metro # color bars with urban_metro groups
        )  |>
  e_error_bar(lower, upper,
              tooltip=list(show= FALSE) # don't show errorbar data in tooltip
              )  |> 
  e_color(psrc_colors$pognbgy_5)  |>
  e_axis_labels(x = "rgc names", y = "vmt")  |>
  e_x_axis(axisLabel=list(rotate= 45)) |> # rotate x-axis label
  e_grid(containLabel= TRUE) |>  # avoid cutting off x-axis label
  e_title("vmt distribution")  |> 
  e_tooltip(formatter = htmlwidgets::JS("
      function(params){
...
test %>% 
  arrange(vmt_day_mean) %>%
  select(urban_metro,final_home_rgcnum,vmt_day_mean,sample_size,lower,upper) %>%
  mutate(vmt_day_mean = round(vmt_day_mean,0)) |> 
  ...
  e_error_bar(lower, upper,
              ...
              width='60%') |> 
  e_color(psrc_colors$pognbgy_5) |>
  e_axis_labels(x = "county", y = "mean of vmt") |>
