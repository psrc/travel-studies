...
rgcs_tracts_list <- read_csv("C:/Joanne_PSRC/DATA/rgc_tracts.csv") %>% select(-1) %>%
  inner_join(read_csv("C:/Joanne_PSRC/DATA/urban_metro.csv") %>% select(3,13), 
             by = "name") %>%
  rename(urban_metro = category)
urban_metro <- read_csv("C:/Joanne_PSRC/DATA/urban_metro.csv")
