...
trip_data_17_19<-trip_data_17_19[,!duplicated(colnames(trip_data_17_19))]%>%mutate(numwkdays=as.numeric(nwkdays))%>%filter(age_category != 'Under 18 years')

dsurvey_1719 <- get_hhts(survey = "2017_2019", 
                       level = "d", 
                       vars = c('age_category'))%>%filter(age_category !='Under 18 years')%>%filter(daynum==1)
```
...
driving_data<-trip_data_17_19 %>% dplyr::filter(mode_simple=='Drive') %>%
  ...
  mutate(weighted_vmt= trip_path_distance*trip_adult_weight_2017_2019/travelers_total)
```

all observed days
```{r}
valid_days_17_19<-dsurvey_1719%>%
                            mutate(vmt_all=0)%>%mutate(weighted_vmt_all=0)%>%
                    filter(age_category!='Under 18 years')%>%filter(day_weight_2017_2019>0)%>%filter(daynum==1)

```

# vmt per day, for days with vmt
```{r}
...
```
```{r}
days_vmt<- left_join(valid_days_17_19, vmt_per,by =c('person_id', 'daynum'))%>%mutate(vmt_day=replace_na(vmt_day,0),weighted_vmt=replace_na(weighted_vmt,0))%>%
  mutate(vmt_every=vmt_all+vmt_day, weighted_vmt_every=weighted_vmt_all+weighted_vmt)
...
```{r}
persons_vmt<-right_join(persons_access,days_vmt, by='person_id')%>%
  mutate(hh_weight_2017_2019_adult=replace_na(hh_weight_2017_2019_adult,0))%>%
  filter(hh_weight_2017_2019_adult>0)%>%
  mutate(ln_vmt_every=log(1+vmt_every))%>%
  filter(vmt_every<400)

```

```{r}
summary(persons_vmt$vmt_every)
summary(persons_vmt$ln_vmt_every)
ggplot(persons_vmt, aes(ln_vmt_every))+
geom_histogram()+
xlim(0, 10)

ggplot(persons_vmt, aes(vmt_every))+
geom_histogram()
```
...
dtelework_1719<-get_hhts(survey = survey_a$survey, level="d", vars=c('telework_time'))
```
...
```{r}
persons_vmt_del1719<-left_join(persons_vmt_new, delivery_1719, by=c('household_id.x'= 'household_id', 'daynum'='daynum'))
persons_vmt_del1719<-left_join(persons_vmt_del1719, dtelework_1719, by=c('person_id.x'='person_id', 'daynum'='daynum'))
...
```{r}
trip_data_21<-trip_data__21[,!duplicated(colnames(trip_data__21))]
```
# calculate vmt 
```{r}
...
driving_data_21<-trip_data_21 %>% dplyr::filter(mode_simple=='Drive') %>%
  ...
  mutate(weighted_vmt= trip_path_distance*trip_adult_weight_2021/travelers_total)
```

all observed days
```{r}
valid_days_21<-dsurvey_21%>% 
                            mutate(vmt_all=0)%>%mutate(weighted_vmt_all=0)
...
```{r}
days_vmt_21<- left_join(valid_days_21, vmt_per_21,by =c('person_id'))%>%mutate(vmt_day=replace_na(vmt_day,0),weighted_vmt=replace_na(weighted_vmt,0))%>%
  mutate(vmt_every=vmt_all+vmt_day, weighted_vmt_every=weighted_vmt_all+weighted_vmt)
...
```{r}
persons_vmt_21<-merge(persons_access_21,days_vmt_21, by='person_id')%>%
  mutate(person_adult_weight_2021=replace_na(person_adult_weight_2021.x,0))%>%
  filter(person_adult_weight_2021>0)%>%
  mutate(ln_vmt_every=log(1+vmt_every))%>%
  filter(vmt_every<400)
...
```{r}
summary(persons_vmt_21$vmt_every)
summary(persons_vmt_21$ln_vmt_every)
ggplot(persons_vmt_21, aes(ln_vmt_every))+
geom_histogram()+
xlim(0, 10)
```
...
dtelework_21<-get_hhts(survey = survey_b$survey, level="d", vars=c('telework_time'))
```

```{r}
persons_vmt_del21<-left_join(persons_vmt_new_21, delivery_21, by=c('household_id.x'= 'household_id'))
persons_vmt_del21<-left_join(persons_vmt_del21, dtelework_21, by=c('person_id.x'='person_id'))
```
...
persons_vmt_17_19_21%>%group_by(survey_year)%>%summarize(mean_vmt=mean(vmt_every))
...
vmt_summary <- function(person_day_df_1, person_weight_name_1,survey_year_1, 
  ...
  vmt_mean_group_1<-person_day_df_1%>%
    ...
    summarize(sum_per_weights=sum(!!ensym(person_weight_name_1)), sum_weighted_vmt=sum(weighted_vmt_every))%>%
    mutate(vmt_mean=sum_weighted_vmt/sum_per_weights)%>%
    mutate(survey_year=survey_year_1)
  
    vmt_mean_group_2<-person_day_df_2%>%
    group_by(!!ensym(group))%>%
    summarize(sum_per_weights=sum(!!ensym(person_weight_name_2)), sum_weighted_vmt=sum(weighted_vmt_every))%>%
    mutate(vmt_mean=sum_weighted_vmt/sum_per_weights)%>%
...
```{r}
employment_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_adult_weight_2021.x', '2021',
...
```{r}
hhinc_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_adult_weight_2021.x', '2021',
                                  'inc100k')
```
...
```
```{r}
vehicle_count_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_adult_weight_2021.x', '2021',
...
```{r}
telework_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_adult_weight_2021.x', '2021',
                                  'telecommute_freq_new')%>%
mutate(telecommute_freq_new=factor(telecommute_freq_new, levels=c("Never or Less than Every Week", "1-4 days per week" ,"5+ days")))%>%
         filter(telecommute_freq_new %in% c("Never or Less than Every Week", "1-4 days per week", "5+ days"))
...
```
```{r}
transitben_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_adult_weight_2021.x', '2021',
                                  'benefits_3')
...
```
something doesn't seem to be working right here
```{r}
delivery_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
                            persons_vmt_del21,'person_respondent_weight_2021.x', '2021',
                                  'delivery_pkgs_all')
```

```{r}
static_column_chart(t=delivery_vmt, x='delivery_pkgs_all', y='vmt_mean', fill='survey_year', 
                    xlabel='package delivery', ylabel='vehicle miles traveled (mean)')
...
```{r}
vmt_model_best_fit_21<-lm(ln_vmt_every ~ employment + inc100k + vehicle_count_simple + 
    gender_grp + numworkers + numchildren + numadults + grad_school + 
    student + license + telecommute_freq_new + Kitsap_home + commute_freq+
...
```{r}
vmt_model_small_fit_21<-lm(ln_vmt_every ~ employment + inc100k + vehicle_count_simple +  numworkers + numadults +  license + telecommute_freq_simple + Kitsap_home + commute_freq+transit_ben + flextime_ben + compressed_ben + rent_own + log(1 + jobs_transit/10000) + log(1 + pharmacy) +
...
```{r}
vmt_mean_group_1<-persons_vmt_del1719%>%
group_by(hhgroup)%>%
summarize(sum_per_weights=sum(hh_weight_2017_2019.x), sum_weighted_vmt_every=sum(weighted_vmt_every))%>%
mutate(vmt_mean=sum_weighted_vmt_every/sum_per_weights)%>%
...
```{r}
vmt_model__small_interest_21<-lm(ln_vmt_every ~ employment+vehicle_count_simple+license+commute_freq+telecommute_freq_simple +transit_ben + compressed_ben + flextime_ben + 
