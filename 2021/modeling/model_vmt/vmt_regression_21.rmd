...
persons_tract<-left_join(persons_list,hh_tract,by='household_id')%>%mutate(tract_char= as.character(final_home_tract10))%>%filter(hhgroup !='rMove')
...
```{r}
persons_vmt_del21<-left_join(persons_vmt_new_21, delivery_21, by=c('household_id.x'= 'household_id', 'daynum'='daynum'))
```
...
```{r}
vmt_model_17_19_21<-lm(ln_vmt_day ~(employment+inc100k+vehicle_count_simple+lifecycle+gender_grp+numworkers+numchildren+numadults+grad_school+
+student+ license+telecommute_freq_simple+ Kitsap_home+transit_ben+compressed_ben+flextime_ben+age+rent_own+ delivery_food_all+delivery_pkgs_all+delivery_grocery_all+ delivery_work_all+ log(1+jobs_auto/10000)+ log(1+jobs_transit/10000)+log(1+pharmacy)+log(1+park)+car_share -1)*survey_year_factor
               ,data=persons_vmt_17_19_21)
```
...
```{r}
output<-stargazer(vmt_model_17_19_21, type='text', out='vmt_model_17_19_21.txt')
```
...
```{r}
stepAIC(vmt_model_17_19_21,direction="both")
```
...
```{r}
vmt_model_best_fit_21<-lm(ln_vmt_day ~ employment + inc100k + vehicle_count_simple + lifecycle + 
    numworkers + grad_school + license + telecommute_freq_simple + 
    Kitsap_home + transit_ben + compressed_ben + flextime_ben + 
    age + rent_own + delivery_pkgs_all + log(1 + jobs_auto/10000) + 
    log(1 + jobs_transit/10000) + log(1 + pharmacy) + log(1 + 
    park) + survey_year_factor + inc100k:survey_year_factor + 
    grad_school:survey_year_factor + transit_ben:survey_year_factor + 
    compressed_ben:survey_year_factor + flextime_ben:survey_year_factor +rent_own:survey_year_factor - 1, data = persons_vmt_17_19_21)
...
```{r}
library(jtools)
summ(vmt_model_best_fit_21)



```
```{r}
plot_summs(vmt_model_best_fit_21)
```
```{r}
vmt_model__small_best_fit_21<-lm(ln_vmt_day ~ employment +  vehicle_count_simple + 
    numworkers + grad_school + license + telecommute_freq_simple +
    Kitsap_home + transit_ben + compressed_ben + flextime_ben + 
    lifecycle +  delivery_pkgs_all + log(1 + jobs_auto/10000) + 
    log(1 + jobs_transit/10000) + log(1 + 
    park) + survey_year_factor , data = persons_vmt_17_19_21)

output<-stargazer(vmt_model__small_best_fit_21, type='text', out='vmt_model_best_21.txt')
summ(vmt_model__small_best_fit_21)
plot_summs(vmt_model__small_best_fit_21)
```
