...
could instead do weighting based on rmove records;
```{r}
...
vmt_model_best_fit_21<-lm(ln_vmt_day ~ employment + inc100k + vehicle_count_simple + 
    ...
    transit_ben + compressed_ben + flextime_ben + rent_own + 
    delivery_work_all + log(1 + jobs_transit/10000) + log(1 + 
    pharmacy) + log(1 + park) + survey_year_factor + workplace+inc100k:survey_year_factor + 
    gender_grp:survey_year_factor + grad_school:survey_year_factor + 
    Kitsap_home:survey_year_factor + transit_ben:survey_year_factor + 
    compressed_ben:survey_year_factor + log(1 + jobs_transit/10000):survey_year_factor + 
...
```
Having transit pass offered is associated with 15% less vmt
teleworking 5 days a week associated with 28% less vmt
delivery no effect measurable
...
```
```{r}
vmt_model_telecommute<-lm(ln_vmt_day ~telecommute_freq_simple* commute_freq+survey_year_factor  , data = persons_vmt_17_19_21)
