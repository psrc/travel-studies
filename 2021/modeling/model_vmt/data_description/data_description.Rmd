...
day_data_17_19<- get_hhts("2017_2019", "d", vars=c())

hh_data_19<- get_hhts("2019", "h", vars=hh_vars) 
per_data_19<- get_hhts("2019", "p", vars=person_vars)
trip_data_19<- get_hhts("2019", "t", vars=trip_vars) 

# calculate vmt data
source("data_description_script.R")
```

# notes for calculating trip weights
...
```{r}
day_vars=c("day_id","recid","pernum","hhsize","vehicle_count","hhincome_detailed","numadults","numchildren","numworkers","lifecycle","education","license","age_category","race_eth_broad","dayofweek","traveldate","survey_year","numtrips","trips_yesno_kid")
# day_weight_2017_2019,day_weight_2017,day_weight_2019,hh_weight_2021,person_adult_weight_2021,person_respondent_weight_2021
day_data_17_19<- get_hhts("2017_2019", "d", vars=day_vars)
day_data_19<- get_hhts("2019", "d", vars=day_vars)
test <- trip_data_19 %>% filter(household_id==191000154, mode_simple=="Drive")
test2 <- per_data_19 %>% filter(household_id==191000154)
test_day <- day_data_19%>% filter(household_id==191000154)
sum(day_data_19$day_weight_2019)
# 17128054
```
```{r}
sum(test2$hh_weight_2019, na.rm = TRUE)
```
```{r}
sum(test_day$day_weight_2019, na.rm = TRUE)
```
