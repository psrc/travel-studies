...
persons_vmt_del21<-left_join(persons_vmt_del21, dtelework_21, by=c('person_id.x'='person_id', 'daynum'='daynum'))
```
...
```{r}
employment_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019.x', '2017/2019',
...
static_column_chart(t=employment_vmt, x='employment', y='vmt_mean', fill='survey_year', 
                    xlabel='employment status', ylabel='vehicle miles traveled (mean)')
...
telework_vmt<-vmt_summary(persons_vmt_del1719,'hh_weight_2017_2019_adult', '2017/2019',
  ...
  mutate(telecommute_freq_new=factor(telecommute_freq_new, levels=c("NA","Never / None", "A few times per month", "Less than monthly", "1-4 days per week" ,"5+ days"  )))
...
static_column_chart(t=telework_vmt, x='telecommute_freq_new', y='vmt_mean', fill='survey_year', 
                    xlabel='Telecommute Frequency', ylabel='vehicle miles traveled (mean)')
...
group_by(hhgroup)%>%
summarize(sum_per_weights=sum(hh_weight_2017_2019.x), sum_weighted_vmt=sum(weighted_vmt))%>%
mutate(vmt_mean=sum_weighted_vmt/sum_per_weights)%>%
