...
header-includes:
  ...
  - \usepackage{graphicx}
   ...
   \fancyhead[R]{}
   \fancyfoot[L]{\scriptsize{1011 Western Ave, Suite 500, Seattle WA 98104} \textcolor[HTML]{F05A28}. 206.464.7532 \textcolor[HTML]{F05A28}. www.psrc.org \textcolor[HTML]{F05A28}. March 2023}
...
times_17 <- get_telecommute_data(survey = "2017",
                                 stat_var = "telecommute_freq_new",
                                 group_vars = "telecommute_freq_new",
                                 weight = "hh_weight_2017",
                                 incl_na = FALSE)
...
filter(telecommute_freq_new %in% c("Never / None", "1-4 days per week", "5+ days"))
...
travel_by_race_21 <- get_telecommute_data(survey = "2021",
                                          ...
                                          incl_na = FALSE) %>% 
  filter(workplace_travel != "Total") %>% 
  mutate(race_category=case_when(race_category=="White Only" ~ "White",
                                 TRUE~as.character(race_category)))
...
travel_by_race_21_poc <- get_telecommute_data(survey = "2021",
                                          stat_var = "workplace_travel",
                                          group_vars = c("workplace_travel", "race_group"),
                                          weight = "person_adult_weight_2021",
                                          incl_na = FALSE) %>% 
  filter(workplace_travel != "Total")
...
travel_21 <- get_telecommute_data(survey = "2021",
                                  ...
                                  incl_na = FALSE) %>%
  filter(workplace_travel != "Total") %>% 
  mutate(race_group = "Region", .before = workplace_travel)

travel_by_race_21_poc <- bind_rows(travel_by_race_21_poc, travel_21)

travel_by_race_21_poc$race_group <- factor(travel_by_race_21_poc$race_group,
                                          levels = c("Non-POC",
                                                     "POC",
                                                     "Region"))
...
```
### Working at home increased from 6% to 27% between 2017 and 2021
```{r workplace, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(7, 3.55)}
...
openxlsx::addWorksheet(wb, sheetName = "Work Location")
openxlsx::writeDataTable(wb, sheet = 1, x = workplace, tableStyle = "none", withFilter = FALSE)
```
...
\fancyhead[L]{}
In the regional travel surveys from 2017 to 2021, respondents with jobs were asked to select their current work location from a list of options. In the 2017/2019 combined data, people who always worked at home comprised only 6\% of all workers in the region. By spring of 2021, the portion of people always working from home increased dramatically to 27\% of all workers in the region. Combined with the portion of people that teleworked some days and traveled to work some days, 37\% of workers in the region worked from home at least part of the time. However, most workers in 2021 still traveled to a work location outside of the home.

## Large changes seen in teleworking
Working remotely from home, also called teleworking, was a feature of the work environment in the Puget Sound region for multiple years before the onset of the COVID-19 pandemic. While it is true that the pandemic led to a major increase in teleworking, we saw that a trend was already underway in the two travel surveys before 2021.

### Yearly increases in weekly rates of teleworking
```{r telecommute_over_year, include=FALSE, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(4,4)}
telecommute_frequency_chart <- static_column_chart(t = telecommute_frequency,
                                                   x = "telecommute_freq_cond", y = "share",
                                                   fill = "survey",
                                                   moe = "share_moe",
                                                   color = "pgnobgy_5",
                                                   title = "Teleworked At Least Once per Week") + theme(axis.text.x = element_text(size = 14),axis.text.y = element_text(size = 14)) + theme(plot.title = element_text(size=16))

# telecommute_frequency_chart
png("telecommute_frequency_chart.png")
plot(telecommute_frequency_chart) 
dev.off()

openxlsx::addWorksheet(wb, sheetName = "Telecommute Frequency")
openxlsx::writeDataTable(wb, sheet = 2, x = telecommute_frequency, tableStyle = "none", withFilter = FALSE)
```
\renewcommand{\columnsep}{25pt}
\begin{wrapfigure}{right}{0.46\textwidth}
  \raggedright
    \includegraphics[width=\linewidth]{telecommute_frequency_chart.png}
\end{wrapfigure}
...
```{r times_per_week, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 5)}
...
openxlsx::addWorksheet(wb, sheetName = "Telecommute Times")
openxlsx::writeDataTable(wb, sheet = 3, x = telecommute_times_per_week, tableStyle = "none", withFilter = FALSE)
```

\newpage

### Telework frequency varied by gender
When telework frequency was broken down by gender, there was little difference in the shares of female workers reporting teleworking 1-2 days per week (11\%), 3-4 days per week (12\%), and 5+ days per week (10\%). There was a slightly larger difference for male workers, with the largest share reporting 5+ days per week (12\%) compared to 9\% and 8\% for 1-2 days per week and 3-4 days per week, respectively. For both genders, the largest increase in teleworking frequency between 2017/2019 and 2021 was for 3-4 days per week; the share of female workers increased from 2\% to 12\% and the share of male workers increased from 3\% to 8\%. There was also a large increase in male workers who reported teleworking 5+ days per week, an increase from 7\% in 2019 to 12\% in 2021.

&nbsp;

```{r telecommute_gender, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 5)}
...
openxlsx::addWorksheet(wb, sheetName = "Telecommute-Gender")
openxlsx::writeDataTable(wb, sheet = 4, x = telecommute_freq_gender, tableStyle = "none", withFilter = FALSE)
```
...
```{r telecommute_race, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 5)}
...
openxlsx::addWorksheet(wb, sheetName = "Telecommute-Race")
openxlsx::writeDataTable(wb, sheet = 5, x = telecommute_freq_race, tableStyle = "none", withFilter = FALSE)
```
...
```{r work_travel_gender, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 3)}
...
openxlsx::addWorksheet(wb, sheetName = "Travel-Gender")
openxlsx::writeDataTable(wb, sheet = 6, x = travel_by_gender_21, tableStyle = "none", withFilter = FALSE)
```
...
```{r work_travel_race, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 3)}
# option 1 - default dodge because 'stack' isn't rendering correctly
...
openxlsx::addWorksheet(wb, sheetName = "Travel-Race")
openxlsx::writeDataTable(wb, sheet = 7, x = travel_by_race_21, tableStyle = "none", withFilter = FALSE)
```

### Differences seen in workplace travel based on worker industry
PSRC first began asking survey respondents what industry they worked in during the 2021 household travel survey. We used industry groups to see differences in workplace travel based on workers' fields of employment. We found that there were seven industry groups (including "Other") with workers reporting that they worked outside the home more than the regional average (63\%; blue bar in chart below): Military (98\%), Hospitality \& Retail (87\%), Personal Services (85\%), Construction \& Resources (82\%), Health Care, Social Services \& Childcare (81\%), Transportation \& Utilities (72\%), and Other (70\%).

&nbsp;

```{r industry_work_travel, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 5)}
...
industry_work_travel_chart_ref <- industry_work_travel_chart +
  ggplot2::annotate("text", x = 1.4, y = 0.92, label = "Regional average", angle='0', color='#00A7A0')

industry_work_travel_chart_ref

openxlsx::addWorksheet(wb, sheetName = "Industry-Travel")
openxlsx::writeDataTable(wb, sheet = 8, x = industry_by_work_loc_21, tableStyle = "none", withFilter = FALSE)
```

\newpage
Of those seven industry groups that worked outside the home more than the regional average of all workers, two had greater proportions of female workers than the regional average (59\% of all workers): Health Care, Social Services \& Childcare (73\%) and Personal Services (69\%).
```{r industry_gender, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 3.25)}
# For reference number - regional proportion of female workers
# travel_by_gender_21
...
industry_gender_chart_ref <- industry_gender_chart +
  ggplot2::annotate("text", x = 1.9, y = 0.61, label = "Regional average", hjust = 0, angle='0', color='#00A7A0') + theme(axis.text.y = element_text(size = 8)) + theme(plot.title = element_text(size=12))

# label = paste("Regional", "\n", "average")

industry_gender_chart_ref

openxlsx::addWorksheet(wb, sheetName = "Industry-Gender")
openxlsx::writeDataTable(wb, sheet = 9, x = industry_by_gender_21, tableStyle = "none", withFilter = FALSE)
```

Of those seven industry groups that worked outside the home more than the regional average of all workers, three had greater proportions of workers of color than the regional average of workers of color (34\% of all workers): Health Care, Social Services \& Childcare (42\%), Hospitality \& Retail (40\%), and Personal Services (39\%).
```{r industry_race, echo=FALSE, results=TRUE, warning=FALSE, fig.dim=c(6, 3.25)}
# For reference number - regional proportion of POC workers
# travel_by_race_21_poc
...
industry_race_chart_ref <- industry_race_chart +
  ggplot2::annotate("text", x = 1.4, y = 0.47, label = "Regional average", angle='0', color='#00A7A0') + theme(axis.text.y = element_text(size = 8)) + theme(plot.title = element_text(size=12))

industry_race_chart_ref

openxlsx::addWorksheet(wb, sheetName = "Industry-Race")
openxlsx::writeDataTable(wb, sheet = 10, x = industry_by_race_21, tableStyle = "none", withFilter = FALSE)
```

```{r output_spreadsheet, echo=FALSE}
openxlsx::saveWorkbook(wb, file = paste0(output_path, "telecommute_trend_story_data.xlsx"), overwrite = TRUE)
```
...
\href{https://www.psrc.org/media/7513}{\underline{\textcolor{blue}{Download the data used in this Trend.}}}