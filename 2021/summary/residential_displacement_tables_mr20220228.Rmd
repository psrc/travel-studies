...
#                          force=TRUE)
# remove.packages("purrr") #needed for tidyr update
# install.packages("purrr") #needed for tidyr update
...
library(plotly) #for subplot function
...
```{r}
# All survey respondents - 2 race categories

#2017
hh_2017<-get_hhts(survey='2017', level="h",
                  vars=c('hh_race_poc'))

hh_2017_counts<- hhts_count(df=hh_2017,
...
counts_2017_2021_2<-rbind(hh_2017_counts, hh_2019_counts, hh_2021_counts) %>% 
  # mutate(type="survey") %>% 
  dplyr::select(-c(share, share_moe, sample_size)) %>% 
  dplyr::rename(total_count=count,
                total_count_moe=count_moe)

# static_column_chart(t=counts_2019_2021_2, x='hh_race_poc', y='share', fill='survey', moe='share_moe',color=  "pognbgy_5")
# 
# static_column_chart(t=counts_2019_2021_2, x='hh_race_poc', y='share', fill='survey',color=  "pognbgy_5")
```

```{r}
# All movers - 2 race categories
...
movers_2017_2021_counts_2<-rbind(movers_hh_2017_counts,
                                 movers_hh_2019_counts, 
                                 movers_hh_2021_counts) %>% 
  # mutate(type="movers") %>% 
  dplyr::select(-c(res_dur, share, share_moe, sample_size)) %>% 
...
# static_column_chart(t=movers_2019_2021_counts_2, x='hh_race_high_low', y='share', fill='survey', moe='share_moe',color=  "pognbgy_5")
# 
# static_column_chart(t=movers_2019_2021_counts_2, x='hh_race_high_low', y='share', fill='survey',color=  "pognbgy_5")
```

```{r}
# Displaced movers - 2 race categories

#2017 - not available in dataset - will generate data based on 2019 dataframe, replacing counts with 0
...
# create dummy 2017 data
displaced_hh_2017_counts <- displaced_hh_2019_counts %>% 
  mutate(survey=2017,
         count=0,
         count_moe=0,
         share=0,
...
displaced_2017_2021_counts_2<-rbind(displaced_hh_2017_counts, 
                                    displaced_hh_2019_counts, 
                                    displaced_hh_2021_counts) %>% 
  # mutate(type="displaced") %>% 
  dplyr::select(-c(prev_res_factors_displaced, share, share_moe, sample_size)) %>% 
...
# static_column_chart(t=displaced_2019_2021_counts_2, x='hh_race_high_low', y='share', fill='survey', moe='share_moe',color=  "pognbgy_5")
# 
# static_column_chart(t=displaced_2019_2021_counts_2, x='hh_race_high_low', y='share', fill='survey',color=  "pognbgy_5")
```
...
race_2_2017_2021 <- cbind(counts_2017_2021_2,
                          movers_2017_2021_counts_2,
                          displaced_2017_2021_counts_2)
```
...
race_2_2017_2021 <- race_2_2017_2021 %>% 
  dplyr::select(unique(colnames(.)))
...
test_calculate <- race_2_2019_2021 %>% 
  ...
  dplyr::select(survey, hh_race_poc, non_mover_perc, mover_perc, disp_perc)

test_calculate <- race_2_2017_2021 %>% 
  dplyr::mutate(mover_perc=(mover_count/total_count)) %>% 
  dplyr::mutate(disp_perc=(displaced_count/total_count)) %>%
  dplyr::mutate(non_mover_perc=((total_count/total_count)-(mover_perc+disp_perc))) %>%
  dplyr::select(survey, hh_race_poc, non_mover_perc, mover_perc, disp_perc) %>% 
  dplyr::mutate(disp_perc= na_if(disp_perc, disp_perc < 0))

test_pivot <- test_calculate %>% 
  pivot_longer(
    cols = c("non_mover_perc", "mover_perc", "disp_perc"), 
    names_to = "type", 
    values_to = "percent") %>% 
  mutate(type=case_when(type=="non_mover_perc"~"did not move",
                        type=="mover_perc"~"movers",
                        type=="disp_perc"~"displaced movers"))
...
```{r}
race_17 <- static_bar_chart(t = df_2017,
                            x = "percent", 
                            y = "hh_race_poc",
                            fill = "type",
                            est = "percent",
...
# ggarrange(race_19, race_21, ncol=1, nrow=2, common.legend = TRUE, legend="bottom")
ggarrange(race_17, race_19, race_21, ncol=1, nrow=3, common.legend = TRUE, legend="bottom")

# interactive
race_17 <- interactive_bar_chart(t = df_2017,
                                 x = "percent", 
                                 y = "hh_race_poc",
                                 fill = "type",
                                 est = "percent",
...
race_21 <- interactive_bar_chart(t = df_2021,
                                 ...
                                 # moe = "share_moe",
                                 color = "pgnobgy_5",
                                 title = "2021")  #%>% layout(showlegend = FALSE)

# subplot(race_19, race_21, nrows = 2, shareX = T) #%>% layout(legend=list(title=list(text='<b> Dataset </b>')))

subplot(race_17, race_19, race_21, nrows = 3, shareX = T)
```
\
\
