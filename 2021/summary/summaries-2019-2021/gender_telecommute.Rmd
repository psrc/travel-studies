...
```{r data summaries by gender, eval=TRUE}
...
```{r data summaries combined gender}
...
gen_tc_freq_2021_tot <- hhts_count(wrkr_2021, group_vars = 'telecommute_freq') %>% 
  mutate(period = survey_b$label) %>% 
  rename(tc_freq_group = telecommute_freq)
...
gender_tcf(plot.source.year = '2017/19, 2021', plot.caption = plot.caption.01, palette = "Paired", hjust = .5, vjust = 0, angle = 0, facet = TRUE)
```

```{r plot simple 2017-19 2021}
# filter df for just 2017/19 2021
df_simple %>% filter(survey %in% c('2017_2019', '2021')) %>% 
gender_tcf(plot.source.year = '2017/19, 2021', plot.caption = plot.caption.01, palette = "Paired", hjust = .5, vjust = 0, angle = 0, facet = TRUE)
```

```{r plot 2017-19 2021 b}
df %>%
...
df %>% 
  ...
  gender_tcf(plot.source.year = '2017, 2019, 2017/19, 2021', plot.caption = plot.caption.01, palette = "Set3", hjust = .5, vjust = 0, angle = 0, facet = TRUE)

```

```{r}
plot.caption.02 <- paste0("\nExcluded 'Prefer not to Answer/Another' gender category and 'Never/None' telecommute responses.")
df_simple %>% 
  filter(tc_freq_group != 'Never / None') %>% 
  filter(gender_group != 'Prefer not to answer / Another') %>% 
  gender_tcf(plot.source.year = '2017, 2019, 2017/19, 2021', plot.caption = plot.caption.02, palette = "Set3", hjust = .5, vjust = 0, angle = 0, facet = TRUE)
```

```{r}
df_simple_tot %>% 
   gender_tcf(plot.source.year = '2017, 2019, 2017/19, 2021', plot.caption = '', palette = "Set3", hjust = .5, vjust = 0, angle = 0, facet = FALSE)
```
```{r}
df_tot %>% 
   gender_tcf(plot.source.year = '2017, 2019, 2017/19, 2021', plot.caption = '', palette = "Set3", hjust = .5, vjust = 0, angle = 0, facet = FALSE)
...
Telecommuting increased between 2017/2019 and 2021 across all major gender groups pictured, however it's not certain that there is a significant difference in increased telecommuting between Females and Males because of overlapping Margins of Error. For those that telecommute at least once a week, there appears to be an almost fair distribution of telecommuters between 1-5 days per week in 2021 with more weight in 1-2 days for Females and 5+ days for Males.
