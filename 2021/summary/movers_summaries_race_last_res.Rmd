...
person_2019<-get_hhts(survey='2019', level="p", vars=c( "prev_home_wa","prev_res_factors_displaced",
...
'race_eth_poc'))%>%filter(prev_res_factors_forced != 'Missing: Skip Logic')
...
mover_sum<- list()
mover_sum_19 <- lapply(vars_to_summarize, FUN=function(x) hhts_count(df=person_2019,group_vars=c('race_eth_poc',x), spec_wgt='hh_weight_2019_v2021'))%>%lapply(FUN=function(y){mutate(y, "var_name"=colnames(y)[3])})%>% rbindlist(use.names=FALSE)%>%filter(prev_res_factors_displaced%in%c('Selected', 'Displaced'))
