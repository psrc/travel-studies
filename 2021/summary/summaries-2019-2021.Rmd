...
gen_tc_freq_2019_simple <- hhts_count(wrkr_2019, group_vars = c('gender_group', 'tc_freq_group_simple')) %>%
  mutate(period = survey_a$label) %>% 
...
gen_tc_freq_2021_simple <- hhts_count(wrkr_2021, group_vars = c('gender_group', 'tc_freq_group_simple')) %>% 
  mutate(period = survey_b$label) %>% 
  rename(tc_freq_group = tc_freq_group_simple)
...
plot_subtitle_inc <- paste('Trips less than', tpd, 'miles')

i <- ggplot(id_1921, aes(x = hhincome_detailed, 
                         y = m, 
                         fill = year,
                         tooltip = paste0(year, ": ", m))) +
  geom_col_interactive(position = position_dodge(width=.9)) +
  geom_linerange(aes(ymin = lower,
                    ymax = upper),
                 # width = 0.2,
  ...
  theme(axis.text.x = element_text(angle = 45,
        ...
        plot.caption = element_text(size=5)) +
  labs(x = NULL,
       y = str_to_title(paste(stat, 'Miles')),
       fill = NULL,
       title = 'Detailed Household Income by Trip Path Distance',
       subtitle = plot_subtitle_inc)
