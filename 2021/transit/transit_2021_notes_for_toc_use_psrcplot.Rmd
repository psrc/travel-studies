...
p<-create_bar_chart(t=trips_by_mode_no_drive, w.x='mode_simple', w.y='share', f='year', w.moe='share_moe', est.type='percent', w.color = 'psrc_pairs')
...
simple_groupings<-c('race_eth_broad'='Race/Ethnicity','hhincome_100_f'='Household Income','edu_simple'= 'Education Level', 'hhsize_simple'= 'Household Size','NoVehicles'= 'Household Vehicles', 'simple_purpose'="Trip Purpose", 'final_home_is_rgc'='Lives in Regional Growth Center', 'gender_grp'='Gender')
...
```{r Updated ggplot2 style and order}
  all_trips_summs_2017_2019 <- hhts_count(transit_data_17_19, 
                                        ...
                                        spec_wgt='trip_weight_2017_2019_v2021_adult')%>%
    filter(mode_simple!='Total')%>%filter(simple_purpose!='Total')%>%filter(mode_simple!='Other')%>%filter(mode_simple!='Bike')
