...
drop table if exists HHSurvey.tblHouseholdAudit

CREATE TABLE [HHSurvey].[tblHouseholdAudit](
	[Type] [char](1) NULL,
...
drop table if exists HHSurvey.tblPersonAudit

CREATE TABLE [HHSurvey].[tblPersonAudit](
	[Type] [char](1) NULL,
	[recid] [bigint] NOT NULL,
	[FieldName] [varchar](128) NULL,
	[OldValue] [nvarchar](max) NULL,
...
drop table if exists HHSurvey.tblDayAudit

CREATE TABLE [HHSurvey].[tblDayAudit](
	[Type] [char](1) NULL,
	[recid] [bigint] NOT NULL,
	[FieldName] [varchar](128) NULL,
	[OldValue] [nvarchar](max) NULL,
...
create  trigger [HHSurvey].[tr_Household] on [HHSurvey].[Household] for insert, update, delete
as
...
declare @bit int ,
    ...
    -- in Household that is edited.  This variable is formatted to be used as part of the SELECT clause in the query 
    ...
    begin
        ...
        begin
            ...
            begin
                select @sql =       'insert into HHSurvey.tblHouseholdAudit (Type, recid, FieldName, OldValue, NewValue, UpdateDate, UserName)'
...
ALTER TABLE [HHSurvey].[Household] ENABLE TRIGGER [tr_Household]
GO



create  trigger [HHSurvey].[tr_Person] on [HHSurvey].[Person] for insert, update, delete
as
...
declare @bit int ,
    ...
    -- Get primary key select for insert.  @PKSelect will contain the recid info defining the precise line
    -- in Person that is edited.  This variable is formatted to be used as part of the SELECT clause in the query 
    -- (below) that inserts the data into.
    select  @PKSelect = coalesce(@PKSelect+',','') + 'convert(varchar(100),coalesce(i.[' + COLUMN_NAME +'],d.[' + COLUMN_NAME + ']))' 
        from    INFORMATION_SCHEMA.TABLE_CONSTRAINTS pk ,
            INFORMATION_SCHEMA.KEY_COLUMN_USAGE c
    ...
    begin
        ...
        begin
            ...
            begin
                select @sql =       'insert into HHSurvey.tblPersonAudit (Type, recid, FieldName, OldValue, NewValue, UpdateDate, UserName)'
                select @sql = @sql +    ' select ''' + @Type + ''''
                select @sql = @sql +    ',' + @PKSelect
                select @sql = @sql +    ',''' + @fieldname + ''''
                select @sql = @sql +    ',convert(varchar(max),d.[' + @fieldname + '])'
...
ALTER TABLE [HHSurvey].[Person] ENABLE TRIGGER [tr_Person]


create  trigger [HHSurvey].[tr_Day] on [HHSurvey].[Day] for insert, update, delete
as
...
declare @bit int ,
    ...
    -- Get primary key select for insert.  @PKSelect will contain the recid info defining the precise line
    -- in Day that is edited.  This variable is formatted to be used as part of the SELECT clause in the query 
    -- (below) that inserts the data into.
    select  @PKSelect = coalesce(@PKSelect+',','') + 'convert(varchar(100),coalesce(i.[' + COLUMN_NAME +'],d.[' + COLUMN_NAME + ']))' 
        from    INFORMATION_SCHEMA.TABLE_CONSTRAINTS pk ,
            INFORMATION_SCHEMA.KEY_COLUMN_USAGE c
    ...
    begin
        ...
        begin
            ...
            begin
                select @sql =       'insert into HHSurvey.tblDayAudit (Type, recid, FieldName, OldValue, NewValue, UpdateDate, UserName)'
                select @sql = @sql +    ' select ''' + @Type + ''''
                select @sql = @sql +    ',' + @PKSelect
                select @sql = @sql +    ',''' + @fieldname + ''''
                select @sql = @sql +    ',convert(varchar(max),d.[' + @fieldname + '])'
...
ALTER TABLE [HHSurvey].[Day] ENABLE TRIGGER [tr_Day]
GO



CREATE TABLE [HHSurvey].[tblVehicleAudit](
	[Type] [char](1) NULL,
	[recid] [bigint] NOT NULL,
	[FieldName] [varchar](128) NULL,
	[OldValue] [nvarchar](max) NULL,
	...
	[UserName] [varchar](128) NULL
		...
		create  trigger [HHSurvey].[tr_Vehicle] on [HHSurvey].[Vehicle] for insert, update, delete
		as
		...
		declare @bit int ,
		    ...
		    -- Get primary key select for insert.  @PKSelect will contain the recid info defining the precise line
		    -- in Vehicle that is edited.  This variable is formatted to be used as part of the SELECT clause in the query 
		    -- (below) that inserts the data into.
		    select  @PKSelect = coalesce(@PKSelect+',','') + 'convert(varchar(100),coalesce(i.[' + COLUMN_NAME +'],d.[' + COLUMN_NAME + ']))' 
		        from    INFORMATION_SCHEMA.TABLE_CONSTRAINTS pk ,
		            INFORMATION_SCHEMA.KEY_COLUMN_USAGE c
		    ...
		    begin
		        ...
		        begin
		            ...
		            begin
		                select @sql =       'insert into HHSurvey.tblVehicleAudit (Type, recid, FieldName, OldValue, NewValue, UpdateDate, UserName)'
		                select @sql = @sql +    ' select ''' + @Type + ''''
		                select @sql = @sql +    ',' + @PKSelect
		                select @sql = @sql +    ',''' + @fieldname + ''''
		                select @sql = @sql +    ',convert(varchar(max),d.[' + @fieldname + '])'
...
ALTER TABLE [HHSurvey].[Vehicle] ENABLE TRIGGER [tr_Vehicle]
GO


CREATE TABLE [HHSurvey].[tblLocationAudit](
	[Type] [char](1) NULL,
	[recid] [bigint] NOT NULL,
	[FieldName] [varchar](128) NULL,
	[OldValue] [nvarchar](max) NULL,
...
GO
		...
		create  trigger [HHSurvey].[tr_Location] on [HHSurvey].[Location] for insert, update, delete
		as
		...
		declare @bit int ,
		    ...
		    -- Get primary key select for insert.  @PKSelect will contain the recid info defining the precise line
		    -- in Location that is edited.  This variable is formatted to be used as part of the SELECT clause in the query 
		    -- (below) that inserts the data into.
		    select  @PKSelect = coalesce(@PKSelect+',','') + 'convert(varchar(100),coalesce(i.[' + COLUMN_NAME +'],d.[' + COLUMN_NAME + ']))' 
		        from    INFORMATION_SCHEMA.TABLE_CONSTRAINTS pk ,
		            INFORMATION_SCHEMA.KEY_COLUMN_USAGE c
		    ...
		    begin
		        ...
		        begin
		            ...
		            begin
		                select @sql =       'insert into HHSurvey.tblLocationAudit (Type, recid, FieldName, OldValue, NewValue, UpdateDate, UserName)'
		                select @sql = @sql +    ' select ''' + @Type + ''''
		                select @sql = @sql +    ',' + @PKSelect
		                select @sql = @sql +    ',''' + @fieldname + ''''
		                select @sql = @sql +    ',convert(varchar(max),d.[' + @fieldname + '])'
...
ALTER TABLE [HHSurvey].[Location] ENABLE TRIGGER [tr_Location]