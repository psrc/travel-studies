...
/* STEP 7. */
	...
	/* Remove duplicated home trips generated by the app */
		...
		(SELECT t.recid 
			FROM HHSurvey.trip AS t 
			JOIN 		HHSurvey.trip AS nrior_t ON t.personid = prior_t.personid AND t.tripnum - 1 = prior_t.tripnum AND t.daynum = next_t.daynum
			...
			WHERE t.o_purpose = 1 AND t.d_purpose = 1 AND next_t.recid IS NULL AND ABS(trip.dest_geom.STDistance(prior_t.dest_geom)) < .0009 )
