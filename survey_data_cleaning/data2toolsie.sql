...
SELECT 	t1.personid, t1.hhid,
		t1.tripnum, t1.modes, LEFT(CONVERT(time(0), t1.depart_time_timestamp, 108), 5) AS depart_hhmm, CONCAT(CAST(ROUND(t1.speed_mph,1) AS NVARCHAR(10)),'mph') AS speed, 
...
SELECT 	t1.personid, t1.hhid,
		...
		t1.dest_name, tp.purpose AS dest_purpose, CONCAT(CONVERT(varchar(30), (DATEDIFF(mi, t1.arrival_time_timestamp, t2.depart_time_timestamp) / 60)),'hr',RIGHT('00'+CONVERT(varchar(30), (DATEDIFF(mi, t1.arrival_time_timestamp, t2.depart_time_timestamp) % 60)),2),'min') AS activity_duration_hhmm,
		...
		FROM trip AS t1
			...
			JOIN trip_purpose AS tp ON t1.dest_purpose = tp.purpose_code
		WHERE t1.hhgroup = 2;
GO