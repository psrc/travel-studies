...
full_list_path <- "2025_codebook/cleaning codebook/rsg_variable_description.csv"
cb_2023_path <- "2023_codebook/2023_01_HTS_Codebook_variable_list.csv"

value_labels_path <- "2025_codebook/cleaning codebook/rsg_value_labels.csv"
...
```{r}
# csv list
variable_list_2025 <- read_csv(full_list_path)
final_cb_2023 <- read_csv(cb_2023_path) %>%
  select(-c("is_checkbox", "location", "shared_name"))
variable_list_2025 <- variable_list_2025 %>%
...
new_variables_2025 <- variable_list_2025 %>%
  anti_join(final_cb_2023, by = names(final_cb_2023)) %>%
  mutate(added = '2025')

variable_full_list <- final_cb_2023 %>%
  mutate(added = '2023') %>%
  add_row(new_variables_2025)
```
...
save codebook
```{r}
distinct_list <- check_variable_full_list %>%
  distinct(variable,view,csv)

final_variable_list <- variable_full_list %>%
  full_join(distinct_list, by="variable") %>%
  arrange(variable)

l <- list("variable_list" = final_variable_list) 
openxlsx::write.xlsx(l, file = "2025_codebook/final_variable_list_2025.xlsx")
```
