...
```{r }
hh<- get_query(sql= "select  household_id as hh_id, home_county, hhincome_broad, survey_year, hh_weight,home_rgcname
...
```{r}
vmt_juris<-driving_day_home%>%group_by(survey_year, home_county)%>%summarize(all_vmt=sum(total_vmt))
```

```{r}
person_county<-left_join(person, hh, by='hh_id')%>%rename(survey_year=survey_year.x)
```

```{r}
person_juris<-person_county%>%group_by(survey_year, home_county)%>%summarize(people=sum(person_weight),count_people=n())
```

```{r}
hhjuris<-hh%>%group_by(survey_year, home_county)%>%summarize(count_hh=n())
```


```{r}
vmt_per_person<-merge(vmt_juris, person_juris, by=c('survey_year', 'home_county'))%>%
  mutate(vmt_per_person=all_vmt/people)%>%filter(count_people>200)%>%filter(survey_year=='2023')

```

```{r}
psrcplot::static_bar_chart(vmt_per_person, y='home_county', x='vmt_per_person', fill='survey_year')
```
...
```{r}
hh%>%group_by(home_county, survey_year)%>%summarize( count=n())
```
