...
person_vmt_summary<-persons_vmt%>%group_by(survey_year.x, race_category)%>%
  ...
  filter(survey_year.x %in% c('2019', '2023'))%>%filter(race_category!='Child')%>%rename(survey_year=survey_year.x)


person_race<-person%>%group_by(race_category,survey_year)%>%summarize(people=sum(person_weight))

person_vmt_summary_race<-merge(person_vmt_summary, person_race, by=c('race_category', 'survey_year'='survey_year'))%>%
mutate(vmt_per_capita=total_vmt/people)%>%filter(survey_year=='2023')
```
```{r}
static_bar_chart(person_vmt_summary_race, y='race_category', x='vmt_per_capita', fill='survey_year')
```
