...
telecommute_or_not_race<- summarize_weighted(hts_data= worker_list,
                               summarize_var = 'telecommute_freq_cond',
                               summarize_by = c('survey_year', 'race_category_2023'),
                               id_cols=c('hh_id', 'person_id'),
                               wt_cols=c('hh_weight', 'person_weight'),
                               wtname= 'person_weight'
                               )
...
telecommute_summary_race<-telecommute_or_not_race$summary$wtd%>%filter(telecommute_freq_cond=='1+ days per week')%>%filter(survey_year=='2023')%>%filter(!race_category_2023 %in% c('Missing/No response'))%>%mutate(moe=prop_se*1.645)
                                                                                                   
static<-static_column_chart(telecommute_summary_race, y='prop', x='race_category_2023', fill='survey_year', moe='moe') + theme(axis.text.x=element_text(size=16), axis.text.y=element_text(size=16),legend.text = element_text(size=16), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))+
   scale_y_continuous(labels = scales::percent_format(scale = 100), limits=c(0,.75))
