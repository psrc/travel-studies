...
static_column_chart(telecommute_summary, y='prop', x='telecommute_status', fill='survey_year', xlabel='Work Location Status', ylabel='Worker Share') + theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
...
```{r}
workers<-person%>%filter(!workplace %in% c('Missing: Skip logic', "Missing: Skip Logic"))%>%drop_na(workplace)
day_telework<-day%>%filter(!telework_time_broad %in% c('Missing: Skip logic', "Missing: Skip Logic"))%>%drop_na(telework_time_broad)
...
telecommute_status_day_time<- summarize_weighted(hts_data= person_daylist,
                               summarize_var = 'telework_time_broad',
                               summarize_by = c('telecommute_status','survey_year'),
                               id_cols=c('person_id', 'day_id'),
                               wt_cols=c('person_weight', 'day_weight'),
                               wtname= 'day_weight'
                               )
...
```
```{r}
static_facet_column_chart(t=telecommute_status_day_time$summary$wtd,y='prop', facet='telework_time_broad', fill='survey_year', x= 'telecommute_status' ,ylabel= 'Worker Share', xlabel='Time spent Teleworking')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
