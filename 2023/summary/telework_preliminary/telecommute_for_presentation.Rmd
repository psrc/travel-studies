...
variable_list<-rbind(
    variable_list,
    data.table(
      variable = c("mode_simple", 'dest_purpose_simple', 'telework_time_broad',  'telework_time_23', 'gender_grp','commute_freq_simple', 'commute_freq_all', 'telecommute_freq_simple','telecommute_status', 'telecommute_status_detail', 'telecommute_freq_cond','seattle_bellevue'),
      ...
      description = c("Mode Group", "Trip Purpose", "Telework Hours", "Telework Time Group", "Gender", 'Commute Frequency', 'Commute Frequency', 'Telecommute Frequency', 'Telecommute Status', 'Telecommute Detail', 'Telecommute or not', 'Home City'),
      logic = c('mode aggregation', 'destination aggregation',"telework time aggregation","telework time aggregation", "gender group", 'commute frequency aggregation', 'commute frequency aggregation', 'telecommute frequency group', "telecommute status", 'telecommute frequency group detail', 'telework or not',  'home in Seattle or Bellevue'),
      ...
      shared_name = c("mode_simple", 'dest_purpose_simple', 'telework_time_broad', 'telework_time_23','gender_grp', 'commute_freq_simple','commute_freq_all','telecommute_freq_simple', 'telecommute_status', 'telecommute_status_detail', 'telecommute_freq_cond', 'seattle_bellevue')
    )
  )
...
static_column_chart(telecommute_summary, y='prop', x='telecommute_status', fill='survey_year', xlabel='Telecommute Status') + theme(axis.text.x=element_text(size=16), axis.text.y=element_text(size=16),legend.text = element_text(size=20))
...
static_column_chart(commute_summary, y='prop', x='commute_freq_all', fill='survey_year', ylabel= 'Worker Share', xlabel='Commute Frequency') + theme(axis.text.x=element_text(size=20), axis.text.y=element_text(size=20),legend.text = element_text(size=20))
...
```{r}
workers<-person%>%filter(!telecommute_freq_simple %in% c('Missing: Skip logic', "Missing: Skip Logic"))%>%drop_na(telecommute_freq_simple)%>%mutate(telecommute_freq_simple=factor(telecommute_freq_simple, levels=c("Never or less than weekly", '1-2 days', '3-4 days', '5+ days')))
worker_list<-list(person=workers)
...
static_column_chart(telecommute_freq_simple_summary, y='prop', x='telecommute_freq_simple', fill='survey_year', ylabel= 'Worker Share, of those who do not always work at home', xlabel='Telommute Frequency') + theme(axis.text.x=element_text(size=20), axis.text.y=element_text(size=20),legend.text = element_text(size=20))
...
```{r}
day_23<- day%>%filter(telework_time!='Missing Response')%>%filter((telework_time_broad!='NA'))%>%
  mutate(telework_time_broad=factor(telework_time_broad, levels=c('0 hours','Less than 1', '1-6 hours', 'More than 6 hours'
                                                                                                                    )
                                                                                                                                     ))
worker_23<-workers
person_daylist<-list(person=worker_23, day=day_23)
```
...
```
```{r}
static_column_chart(t=telecommute_status_day_time$summary$wtd,y='prop', x='telework_time_broad', fill='survey_year')
```
```
