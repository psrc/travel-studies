...
also run telecommute_july23_part3
...
output_trips<-output$summary$wtd%>%filter(dest_purpose_simpler!='Go Home')
output_trips_wrker<- left_join(output_trips, output_wrkr_sum, by='telecommute_status')%>%filter(dest_purpose_simpler!='Go Home')
output_trips_wrker<-output_trips_wrker%>%mutate(trips_per_worker=est.x/est.y)%>%mutate(trips_per_worker_per_week= trips_per_worker*5)%>%mutate(dest_purpose_simpler=fct_reorder(dest_purpose_simpler, trips_per_worker))%>%mutate(telecommute_status=factor(telecommute_status, levels=c('Fully In Person', 'Hybrid', 'Fully At Home')))%>%filter(dest_purpose_simpler!='Go Home')
...
output_trips_wrker<-output_trips_wrker%>%mutate(mode_characterization=fct_reorder(mode_characterization, prop.x))%>%mutate(telecommute_status=factor(telecommute_status, levels=c('Fully In Person', 'Hybrid', 'Fully At Home')))
