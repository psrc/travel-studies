...
```{r}
triprate_prep<-hts_prep_triprate(summarize_by = c('dest_purpose_simple'),
                    variables_dt = variable_list,
...
output <- hts_summary(triprate_prep$num,
                       summarize_var = 'num_trips_wtd',
                       summarize_by = c('dest_purpose_simple'),
                       summarize_vartype = 'numeric',
                       id_cols = ids,
                       wtname = 'day_weight',
                       se=TRUE )
...
```{r}
wtd_trip_rate_23<-output$summary$wtd
wtd_trip_rate_23<-output$summary$wtd%>%mutate(times=5*mean)%>%filter(dest_purpose_simple!='NA')%>%
                                                                    
  mutate(dest_purpose_simple= fct_reorder(dest_purpose_simple,-times))%>%mutate(survey_year='2023')
```
...
wtd_trip_rate_19<-output$summary$wtd%>%mutate(times=5*mean)%>%filter(dest_purpose_simple!='NA')%>%
                                                                    
  mutate(dest_purpose_simple= fct_reorder(dest_purpose_simple,-times))%>%mutate(survey_year='2019')
```
...
wtd_trip_rate_21<-output$summary$wtd%>%mutate(times=5*mean)%>%filter(dest_purpose_simple!='NA')%>%
                                                                    
  mutate(dest_purpose_simple= fct_reorder(dest_purpose_simple,-times))%>%mutate(survey_year='2021')
```

```{r}
z_score<-1.65
wtd_trip_rate_all<-rbind(wtd_trip_rate_19, wtd_trip_rate_21, wtd_trip_rate_23)%>%filter(dest_purpose_simple!='Home')%>%
  mutate(moe_time=5*z_score*mean_se)
...
z_score<-1.645
mean_dist_summ<-mean_dist_summ%>%mutate(moe=z_score*mean_se)
static_column_chart(t=mean_dist_summ,y='mean', x='survey_year', fill='dest_purpose_simple' ,ylabel= 'Mean Distance', xlabel='Trip Destination Purpose', dec=0, moe='moe')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=14), axis.title.x=element_text(size=14))+ylim(0, 6)
