...
values_label<-add_values_code(group_name='gender_grp')
...
# rename all variables based on their nice names
```{r}
nice_variable_names<-variable_list%>%select(variable, description)

summary_out<-left_join(summary_out,nice_variable_names, join_by(var_1_name==variable))%>%rename(var_1_nice_name=description)

summary_out<-left_join(summary_out,nice_variable_names, join_by(var_2_name==variable))%>%rename(var_2_nice_name=description)%>%mutate(category_1= 'Test1', category_2='Test2')

```

```{r}
write.csv(summary_out, 'summary_out.csv')
```
