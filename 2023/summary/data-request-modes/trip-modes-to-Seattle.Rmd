...
trips by mode home county by destination jurisdiction
```{r}
output <- summarize_weighted(hts_data= hh_data_23,
                               summarize_var = 'mode_characterization',
                               summarize_by = c('home_county', 'dest_jurisdiction'),
                               id_cols=ids,
                               wt_cols=wts,
                               wtname= 'trip_weight'
                               )
...
```{r}
write.csv(output$summary$wtd, 'home_county_dest_jurisdiction_mode.csv')
```
...
output <- summarize_weighted(hts_data= hh_data_23,
                               summarize_var = 'mode_characterization',
                               summarize_by = c('home_county', 'dest_county'),
                               id_cols=ids,
                               wt_cols=wts,
                               wtname= 'trip_weight'
                               )
...
```{r}
write.csv(output$summary$wtd, 'home_county_dest_county_mode.csv')
```
...
```{r}
write.csv(output$summary$wtd, 'home_county_mode.csv')
```

```{r}
work_trips<-trip_23%>%filter(dest_purpose %in% c('	
...
```{r}
hh_data_23<-
  list(hh=hh_23, person=person_23, day=day_23, trip=work_trips)
```
...
```{r}
write.csv(output$summary$wtd, 'home_county_mode_work.csv')
...
```
```{r}
write.csv(mode_to_work, 'acs_mode_to_work.csv')
```
