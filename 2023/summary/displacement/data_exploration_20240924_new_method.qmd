...
vars <- c(
  ...
  'prev_res_factors_crime', 'prev_res_factors_employment', 'prev_res_factors_forced',
  'prev_res_factors_hh_size', 'prev_res_factors_housing_cost',
  'prev_res_factors_income_change', 'prev_res_factors_less_space',
  'prev_res_factors_more_space', 'prev_res_factors_no_answer', 
  'prev_res_factors_other', 'prev_res_factors_quality', 
  'prev_res_factors_school', 'prev_res_factors_specify', 
  'prev_res_factors_telework', 'hh_race_category',
  ...
  'ethnicity_999', 'ethnicity_other',
  'person_id', 'gender', 'age',
  'education', 'employment')

# Retrieve the data
hts_data <- get_psrc_hts(survey_vars = vars)  # default includes all survey_years
# hts_data_p <- get_psrc_hts(survey_vars = p_vars)  # default includes all survey_years

# hh <- data.frame(hts_data_hh$hh)
# person <- data.frame(hts_data_p$person)
```
...
Based on a denominator of 1286 in 2023, we can look at displacement.  
```{r}
...
# create chart
vis_all <- static_column_chart(filter(disp_allrace, survey_year == focus_year, 
                                      displaced=="Displaced"),
...
# create chart
vis_4 <- static_column_chart(filter(disp_4race, survey_year == focus_year, 
                                    displaced=="Displaced"),
...
# create chart
vis_3 <- static_column_chart(filter(disp_3race, survey_year == focus_year, 
                                    displaced=="Displaced"),
...
# create chart
vis_2 <- static_column_chart(filter(disp_2race, survey_year == focus_year, 
                                    displaced=="Displaced"),
...
# Calculate a categorical summary, i.e. count/share
disp_inc3 <- psrc_hts_stat(hts_data, 
                                  analysis_unit="hh", 
                                  group_vars=c("hhincome_bin3", "displaced"))

focus_year <- 2023

# view data
head(dplyr::select(disp_inc3[survey_year==focus_year],
                   -c(survey_year)))

# create chart
static_column_chart(filter(disp_inc3, 
                           survey_year == focus_year),
                    x = "hhincome_bin3", y = "prop", fill = "displaced",
...
### Transition
Using the same aggregation categories as with the previous housing type. 
"Other" == Dorm or institutional housing; Mobile home/trailer; Other (including boat, RV, van, etc.)
```{r}
...
# create chart
static_facet_column_chart(filter(disp_rgc, 
                                 survey_year == focus_year),
                          x = "res_type_simp", y = "prop", fill = "displaced",
                          facet="RGC", pos="stack",
