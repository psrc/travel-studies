...
```{r}
# # read hts data
# vars <- c("hhsize","home_jurisdiction","home_lat","home_lng")
# hts_data <- get_psrc_hts(survey_vars = vars,survey_years = 2023)

# create household home location layer
hh <- hts_data$hh %>%
  st_as_sf(coords = c("home_lng", "home_lat"), 
           crs = 4326)

test <- hh %>% filter(is.na(home_lat) | is.na(home_lng))
head(hts_data$hh$home_lng)

# create regional geography layer
region_geo <- st_read_elmergeo('REGIONAL_GEOGRAPHIES')
my_layer <-  region_geo %>% group_by(class_desc) %>% summarise()

# spatial join
hh_region_geo <- hh[c(1:10),] %>% st_join(my_layer, join = st_within)
st_geometry(hh_region_geo) <- NULL
```