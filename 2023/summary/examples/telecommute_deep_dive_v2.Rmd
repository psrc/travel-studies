...
Remote Workers
Never go to job location(s) or at least less than once a week


Hybrid Workers
Any worker who teleworks more than once a week and also goes to a job location at least once per week
( Can include Split-day hybrid workers- Workers who telework and go to work in the same day)

Onsite Workers
Those who go to a job location at least once a week, telework never or less than weekly
...
case_when(workplace %in%c('Missing: Skip logic', "Missing: Skip Logic") 
         ...
         commute_freq_simple == "Never or less than weekly")
          ~ 'Remote',
         (telecommute_freq_cond == 'Less than 1 day per week' & commute_freq_simple != "Never or less than weekly")
          ~' Infrequent Hybrid',
          (telecommute_freq_cond == '1+ days per week' & commute_freq_simple != "Never or less than weekly")
          ~' Frequent Hybrid',
          .default = 'Onsite'
)
)
...
telecommute_summ_complex <- summarize_weighted(hts_data= worker_list,
                               summarize_var = 'telecommute_freq',
                               summarize_by = c('survey_year', 'commute_freq'),
                               id_cols=c('person_id'),
...
telecommute_summary_complex<-telecommute_summ_complex$summary$wtd
static_facet_column_chart(telecommute_summary_complex, y='prop', x='commute_freq', fill='telecommute_freq', facet='survey_year', xlabel='Telecommute Status') 
...
telecommute_freq_simple_gender_summary<-telecommute_time$summary$wtd
static_bar_chart(telecommute_freq_simple_gender_summary, y='telecommute_time_23', x='prop', fill='workplace')
