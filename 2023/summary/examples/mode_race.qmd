...
```{r}
variable_list<-add_variable(variable_list, 'race_cat2','person')
group_labels<-get_grouped_labels(group_id='group_2', group_name='race_cat2')
value_labels<-add_values_code(group_name='race_cat2')
person<-grp_to_tbl(tbl=person, ungrouped_name='race_category', grouped_name='race_cat2')
```
...
mode_summary = summarize_weighted(hts_data= hts_data,
                               summarize_var = 'mode_char1',
                               summarize_by = c('survey_year', 'race_cat2'),
                               id_cols= ids,
                               wt_cols=wts,
                               wtname='trip_weight'
                               )
...
common_modes<-mode_summary$summary$wtd%>%
  mutate(mode_char1= fct_reorder(mode_char1,-prop))%>%drop_na(mode_char1)%>%mutate(moe=prop_se*1.645)%>%filter(!(race_cat2%in% c('Missing', 'Child')))

static_facet_column_chart(common_modes, y='prop', x='survey_year', fill='mode_char1',ylabel= 'Mode Share', xlabel='Year', moe='moe', facet='race_cat2')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
write.csv(common_modes, 'mode_trend.csv')
```
...
wts = c('hh_weight', 'person_weight')

```

```{r}

person_summary = summarize_weighted(hts_data= hts_data,
...
person_race<-person_summary$summary$wtd%>%mutate(moe=prop_se*1.645)%>%
  mutate(race_category= fct_reorder(race_category,-est))

static_bar_chart(person_race, x='est', y='survey_year', fill='race_category',ylabel= 'Weighted Person Estimate', xlabel='Year', color= 'pgnobgy_10', pos='stack')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=8), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
write.csv(common_modes, 'mode_trend.csv')
```
