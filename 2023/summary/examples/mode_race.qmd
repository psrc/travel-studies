...
common_modes<-mode_summary$summary$wtd%>%
  mutate(mode_char1= fct_reorder(mode_char1,-prop))%>%drop_na(mode_char1)%>%mutate(moe=prop_se*1.645)%>%filter(survey_year=='2023')%>%filter(!(race_cat2%in% c('Child', 'Missing/No response')))%>%filter(!(mode_char1 %in% c('SOV', 'HOV')))

static_bar_chart(common_modes, y='race_cat2', x='prop', fill='mode_char1',ylabel= 'Share', xlabel='Trip Mode', , pos='stack')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
write.csv(common_modes, 'mode_trend.csv')
```
