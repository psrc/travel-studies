...
group_labels   <- value_labels%>%
                    filter(group_1_title == 'gender_grp')%>%
                    select('label', group_1_value)

value_labels<-add_values_code(group_labels,  value_labels, 'gender_grp','group_1_value')
...
```{r}
person <-left_join(person, group_labels, by=c('gender' = 'label'))
setnames(person, 'group_1_value', 'gender_grp')
