...
value<- c(0,1,2)
label=c( 'NotRGC','UrbanRGC', 'MetroRGC')
...
trip<-trip%>%mutate(rgc_cat_od= 
                      case_when(is.na(category)& is.na(category.destination) ~ 'NotRGC',
                                category=='Metro' | category.destination=='Metro' ~ 'MetroRGC',
                                category=='Urban' | category.destination=='Urban' ~ 'UrbanRGC',
                                TRUE ~'Missing')
)
```
...
```
```{r}
static_facet_column_chart(trips_summary_wtd, x='work_or_not', y='est', fill='survey_year',ylabel= 'Share', xlabel='Trips', facet='rgc_cat_od',scale='fixed')+ theme(axis.text.x=element_text(size=14), axis.text.y=element_text(size=14),legend.text = element_text(size=14), axis.title.y=element_text(size=20), axis.title.x=element_text(size=20))
```
