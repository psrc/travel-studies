...
person_id, mode_1, dest_purpose,survey_year, travelers_total, origin_rgcname, dest_rgcname, trip_weight 
...
trip<-trip%>%mutate(work_or_not=if_else(dest_purpose %in% work_purposes, 'Work', 'NotWork'))
...
work_modes<-common_modes%>%filter(work_or_not=='Work')
not_work_modes<-common_modes%>%filter(work_or_not=='NotWork')
