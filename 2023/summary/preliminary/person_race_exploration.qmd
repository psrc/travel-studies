...
person<- get_query(sql= "select household_id as hh_id, race_category,
                   ...
                   race_category_2023, race_category_2023_optimized, 
...
adult_23_simp <- adult_23 %>% 
  select(hh_id, person_id, race_category_2023_optimized, age) 
...
hh_2people_wider <- combined %>% 
  filter(n_members==2) %>% 
  pivot_wider(names_from = race_category_2023_optimized, values_from = n_members, values_fill = 0) %>% 
...
combined_race_number <- combined %>% 
  dplyr::mutate(race_num=case_when(race_category_2023_optimized=="White non-Hispanic"~1,
                                   race_category_2023_optimized=="Hispanic"~2,
                                   race_category_2023_optimized=="Black or African American non-Hispanic"~3,
                                   race_category_2023_optimized=="Asian non-Hispanic"~4,
                                   race_category_2023_optimized=="Some Other Race non-Hispanic"~5,
                                   race_category_2023_optimized=="Two or More Races non-Hispanic"~6,
                                   race_category_2023_optimized=="Missing/No response"~7))
...
table1(~race_category_2023_optimized, data=one_person_hh_nonwhite)
```
...
pivot <- other_hh_race %>% 
  pivot_wider(names_from = race_category_2023_optimized, values_from = n_members.x, values_fill = 0) %>% 
