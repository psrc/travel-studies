...
adult_23_simp <- adult_23 %>% 
  select(hh_id, person_id, race_category_2023, age) 

hh_id_multiple_23_prace <- adult_23_simp %>% 
  group_by(hh_id) %>% summarize(n_members=n()) #count how many people per hh - total hh is 3661, total people is 5931
```
...
hh_2people_wider <- combined %>% 
  filter(n_members==2) %>% 
  pivot_wider(names_from = race_category_2023, values_from = n_members, values_fill = 0) %>% 
  group_by(hh_id) %>% summarise(across(any_of(updated_race_list), list(sum = sum))) %>%
