...
mode_summary_rail <- mode_summary |>
  filter(grepl('Urban Rail', mode_1))

table(trip %>% filter(survey_year==2023, grepl('Urban Rail',mode_1)) %>%select(mode_1))

table(trip %>% filter(survey_year==2023, grepl('rail',mode_1)) %>%select(mode_1))
...
static_column_chart(filter(mode_summary, grepl('Urban Rail', mode_1)),
                    x = "survey_year", y = "est", fill = "mode_1",
                    ylabel = "# of Trips", xlabel = "Survey Year", title = "Urban Rail Trips - Estimate",
                    moe = "est_se") + theme(
