...
two_vars_by_hhgroup= function(dataset,col_name,var_string){
  ...
  smartphone_mode = trips_by_var%>% 
    filter(hhgroup %in% c(9, 10,11))%>%
    group_by(hh_is_complete_b)%>%
    rename (smartphone_n_trips = n_trips) %>% 
    mutate(smartphone_trips_share = round(smartphone_n_trips/sum(smartphone_n_trips)*100,1))
  
  temp = browser_mode %>% 
    full_join(smartphone_mode,  by = var_string) %>% 
    dplyr::select(-c(hhgroup.x, hhgroup.y, hh_is_complete_b)) %>% 
...
First we will look at the UNCLEANED data from RSG, then the RULESY cleaned data, and then final cleaned data.

# Uncleaned data- delivered by RSG
...
h <- dbGetQuery(elmer_connection,
                "SELECT * FROM combined_data.v_household")
p <- dbGetQuery(elmer_connection,
                "SELECT * FROM combined_data.v_person")

d <- dbGetQuery(elmer_connection,
                "SELECT * FROM combined_data.v_day")

t <- dbGetQuery(elmer_connection,
                "SELECT * FROM combined_data.v_trip")
```
...
### average trips per person by smartphone vs ABS
...
```{r, echo=FALSE,message=FALSE,warning=FALSE}
day_smartphone_pernum1 = d %>% filter(sample_source == 2, pernum == 1)

var_label_table = as.data.frame(value_labels[value_labels$variable %in% c("sample_source")] %>% mutate(value = as.double(value)))

person_smartphone = subset(p, (personid %in% day_smartphone_pernum1$personid)) 

person_smartphone_summary = person_smartphone %>% 
...
all_ppl = bind_rows(ppl_ABS,person_smartphone_summary)
...
### trips by mode (mode_1) by smartphone vs ABS; and overall
...
temp = ABS_mode %>% 
  full_join(smartphone_mode,  by = "mode_1") %>% 
...
### trips by origin purpose by smartphone vs ABS; overall
...
### trips by destination purpose by smartphone vs ABS; overall
...
Online smartphone Participants are assigned retrospective travel dates (most recent Monday, Tuesday, Wednesday, Thursday) and provide diary information for ONLY person 1. To find share of 0 trips for the smartphone, we need to subset pernum = 1. There are 896 people from the online smartphone that were asked o provide travel diary.
...
person_smartphone_summary_uncleaned<-person_smartphone_summary

person_smartphone_summary_formatted<-get_labels(person_smartphone_summary, sample_source, "sample_source") %>%
  kbl() %>%
  #kable_material(c("striped", "hover"))
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"))
person_smartphone_summary_formatted
...
ppl_with_no_trips_smartphone = subset(person_smartphone, !(personid %in% personid_with_trips$personid))

no_trips_by_ss_ABS = ppl_with_no_trips_ABS %>% 
                group_by(sample_source) %>% 
                summarise(n_ppl_no_trips=n())

no_trips_by_ss_smartphone = ppl_with_no_trips_smartphone %>% 
                group_by(sample_source) %>% 
                summarise(n_ppl_no_trips=n())

no_trips_by_ss = bind_rows(no_trips_by_ss_ABS, no_trips_by_ss_smartphone)

people_with_trips_smartphone = subset(person_smartphone, (personid %in% personid_with_trips$personid)) %>% 
                          group_by(sample_source) %>% 
...
people_with_trips = t %>%
  ...
  bind_rows(people_with_trips_smartphone)
...
person_smartphone_summary_cleaned<-person_smartphone_summary
