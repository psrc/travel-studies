...
# Maintain Variable List

## get all variables in Elmer views

```{r}
# all variables in views
...
## get all variables in csv record

```{r}
variable_full_list <- read_csv(full_list_path)

# all variables in views csv variables
vars_csv <- data.frame()
for(t_name in table_names){
  v_name <- view_names[t_name]
...
df_view_name |>
  ...
  gt()|>
  tab_header(
    title = "Variables counts in each Elmer view and in CSV record"
  )
```
...
inconsistent_vars |>
  gt()|>
  tab_header(
    title = "List of all inconsistent variables"
  )
```
